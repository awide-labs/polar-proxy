...
2. **The request and response stages with file:line** (sections 4 and 5): the same stages, but pinned to the exact functions and lines of this implementation.
...
These terms are defined once here and used the same way throughout. The project glossary in `README.md` is the authority for this docset (the curated glossary lives at `doc/polardb-arch/34-DOCS-FOUNDATION-PACK.md` in the full implementation); this is a working subset.
...
|-------|----------|-----------|--------------|---------------|
| collect | `polardb_collect()` | `lib/PgSQL_PolarDB_Flow.cpp:233` | request path, in the `'Q'` handler | Usually reads inputs; clears stale session LSN targets/latches if the writer group or epoch changed |
| plan | `polardb_plan()` | `lib/PgSQL_PolarDB_Flow.cpp:410` | request path, right after collect | None (may attach lag-cap inputs to the per-query reader plan) |
| execute | `polardb_execute()` | `lib/PgSQL_PolarDB_Flow.cpp:658` | request path, right after plan | Yes — sets the target HG, stages the wait state |
| process_result | `polardb_process_result()` | `lib/PgSQL_PolarDB_Flow.cpp:846` | response path, on success | Yes — advances session write/observed LSN state, maintains missing-LSN latches, refreshes the per-server LSN cache |

Between execute and process_result there is one more PolarDB step that is not a "stage" but is essential: the **wrap finalize** step, `finalize_wait_timeout_injection()` (`lib/PgSQL_PolarDB_Wrap.cpp:303`), which builds the actual wrapped SQL once the backend connection exists.
...
```
 ...
 [collect]  snapshot all routing inputs; repair stale writer epoch state
   │        (may clear old session LSN targets/latches after writer change)
   ▼
...
This is the mental model. It does not name files. It names the questions each stage answers and the data each stage produces. Sections 4 and 5 then pin each of these to code.

### 3.1 Request stages (R-stages)

| Stage | Question it answers | Output it produces |
|-------|--------------------|--------------------|
| **R0 — gate** | Is any PolarDB hostgroup configured at all? Is the user doing manual routing? | If no PolarDB HG is active, or the user manually routed the query, the pipeline does nothing and normal ProxySQL routing applies. |
| **R1 — collect** | What are all the routing inputs right now (topology, mode, timeout, lag cap, this session's LSN targets/latches, writer group/epoch, query shape)? | A routing-input snapshot. If the writer group or epoch changed, stale session LSN targets and missing-LSN latches are cleared before the snapshot is copied. |
...
| **R4 — bind backend** | Which actual backend connection serves this query? | A backend connection in the target hostgroup. For a replica-with-wait, the connection picker prefers a replica caught up to the required LSN. If the picked reader is already at the target (thread-local cache hit or a `wait_bypass_allowed` route-smart acquisition), R4 clears the staged wait and bumps `PolarDB_Wait_Wrap_Bypassed`, so R5 wraps nothing (§4.5). |
...
| **S4 — process_result** | Did this query carry an accepted current-group/current-epoch LSN, and was it a write? | On a write whose RFQ LSN is accepted by the direct HGM update gate, the session write LSN advances. Any accepted LSN-bearing RFQ refreshes the per-server LSN cache. |
...
The actual read-your-writes guarantee comes from **one** thing: the `SET polar_xact_split_wait_lsn = '<target>'` statement in the wrapper makes the replica block until it has caught up. It is **not** enforced by comparing LSNs in the proxy. The proxy's lag cap (a byte bound on how far behind a replica may be) is a **safety** check only — it avoids picking a replica so far behind that the wait would probably time out. If you remember one thing about this pipeline, remember that the wait `SET` is the gate and the lag cap is not (`lib/PgSQL_PolarDB_Flow.cpp:132-140`).
...
- `PgHGM->status.polardb_active` is false (no PolarDB hostgroup loaded). This is a cheap atomic gate set at `lib/PgSQL_HostGroups_Manager.cpp:1864`. (`lib/PgSQL_Session.cpp:2543`)
- **Manual mode**: the query rule did not set `replica_eligible` (it is `-1`, "unset") but did set an explicit `destination_hostgroup`. That means the user routed the query by hand, so the pipeline must not override it (`lib/PgSQL_Session.cpp:2546-2551`).

`replica_eligible` is a tri-state from the matching query rule: `1` = auto (the pipeline decides), `0` = force the writer, `-1` = unset (`lib/PgSQL_Session.cpp:2540-2542`).
...
`polardb_session_consistency.observed_lsn`, and both missing-LSN latches before
copying session state into `route_ctx`; those targets belonged to another replication
group or an old writer/timeline. If any target or latch was actually discarded,
...
`route_ctx.is_polar_hg` guard stops the pipeline.

| Input field | Source | File:line |
|-------------|--------|-----------|
| `is_polar_hg` | HGM config lookup | `:51-52` |
| `writer_hg` | HG config writer (falls back to current HG if `<0`) | `:61-62` |
| `reader_hg` | HG config reader (`-1` if none) | `:63` |
| `effective_consistency_mode` | `polardb_resolve_consistency_mode(session, HG, global)` (session override > per-HG > global) | `:66-69` |
| `wait_timeout_ms` | `polardb_resolve_wait_timeout_ms(HG policy)` | `:72` |
| `wait_timeout_mode` | thread var (best_effort vs strict) | `:73` |
| `max_lag_bytes` | HG policy lag cap | `:74` |
| `writer_epoch` | topology snapshot writer epoch for this replication group | `:66-68` |
...
| `replica_eligible` | `qpo_replica_eligible == 1` | `:80` |
| `is_multi_statement` | semicolon scan of the query, computed only when `replica_eligible` | `:85-96` |
| `is_extended_protocol` | `extended_query_phase != EXTQ_PHASE_IDLE` | `:99` |
| `in_transaction` | `active_transactions > 0` | `:103` |
| `force_primary_hint` | `/* route=primary */` first-comment hint | `:106` |
...
|-------|-----------|--------|-------------|-----------|
| fast | not a PolarDB HG | PASSTHROUGH (target `-1`) | — | `:209-214` |
| fast | no reader HG | PASSTHROUGH (target = writer) | — | `:215-221` |
| fast | not replica-eligible | PASSTHROUGH (target = writer) | — | `:222-228` |
| L0 | `/* route=primary */` hint | FORCE_PRIMARY | HINT_PRIMARY | `:234-241` |
| L1 | mode = PRIMARY_ONLY | FORCE_PRIMARY | MODE_PRIMARY | `:249-256` |
| L1 | mode = OFF | PASSTHROUGH (target `-1`, rules own routing) | — | `:260-266` |
| L2 | in an explicit transaction | FORCE_PRIMARY | IN_TRANSACTION | `:269-276` |
| L2 | multi-statement read | FORCE_PRIMARY | MULTI_STATEMENT | `:277-284` |
| L3 | write/observed/primary target unknown | strict: FORCE_PRIMARY; best_effort: degraded reader only for simple query; extended: FORCE_PRIMARY | WRITE_LSN_UNKNOWN / OBSERVED_LSN_UNKNOWN / PRIMARY_LSN_UNKNOWN | `Flow.cpp:385+` |
| L4 | no wait needed (`session_lsn == 0`) | PASSTHROUGH (target = reader) | — | `Flow.cpp:459+` |
| L5 | extended protocol with a known wait target | FORCE_PRIMARY | EXTENDED_PROTOCOL | `Flow.cpp:472+` |
...
| L7 | otherwise | REPLICA_WITH_WAIT (target = reader) | NONE | `Flow.cpp:497+` |
...
`polardb_execute()` (`lib/PgSQL_PolarDB_Flow.cpp:658`) applies the plan and returns the final target hostgroup. It always first resets the per-query wait state.

- **PASSTHROUGH**: return `plan.target_hg` unchanged (`:388-392`).
- **FORCE_PRIMARY**: set the final target to `route_ctx.writer_hg` (`:394-400`).
- **REPLICA_WITH_WAIT**: two runtime safety overrides first, then prepare the wait:
  - malformed `'Q'` packet (`pkt.size < 7`) → override to the writer (`:408-415`).
  - `polardb_wait_disabled` latch set (an earlier wrap build failed) → override to the writer (`:422-429`).
...
Execute does **not** build the wrapped SQL. It only saves intent. The wrapping is deferred to R5 so it happens exactly once, after a backend exists (`:442-444`).
...
**Wrapper bypass branch.** If the acquired reader is already at the consistency target, R4 clears the staged wait so R5 produces no wrapper. There are two such cases:

- A thread-local cache hit: `get_MyConn_local_polardb_reader()` returns a cached RFQ-capable backend already at the target. The session calls `polardb_query.reset_wait()` and `polardb_query.reset_reader_target()`, bumps `PolarDB_Wait_Wrap_Bypassed` and `PolarDB_TL_Cache_Bypassed_For_Target`, and emits the trace `PolarDB WRAP BYPASS: thread-local reader reached consistency_target_lsn=...`.
- A route-smart acquisition: `get_MyConn_polardb_reader()` returns a connection from the fresh target-reached prefix with `reader_result.wait_bypass_allowed == true`. The session calls `polardb_query.reset_wait()`, bumps `PolarDB_Wait_Wrap_Bypassed`, and emits the trace `PolarDB WRAP BYPASS: route-smart reader reached consistency_target_lsn=...`.

Because `reset_wait()` runs before R5, `finalize_wait_timeout_injection()` sees no active wait and returns CONTINUE without building anything — the bare read goes to the reader already at the consistency target, with no SETs and no `wrapper_stmts` count ([07-QUERY-WRAPPING.md](07-QUERY-WRAPPING.md) §4.5). This is only ever taken for a reader proven to have reached the consistency target; for any other reader the wait wrapper remains the RYW enforcement step (invariant I9 in [14-INVARIANTS-AND-FAILURE-MODES.md](14-INVARIANTS-AND-FAILURE-MODES.md)).

### 4.6 R5 — wrap finalize

At `ASYNC_IDLE`, after the backend connection exists, the session calls `finalize_wait_timeout_injection()` exactly once (`lib/PgSQL_Session.cpp:3607`). This is the single wrapping point.

`finalize_wait_timeout_injection()` (`lib/PgSQL_PolarDB_Wrap.cpp:303`) does, in order:
...
If finalize returns FAILED, the caller does **not** silently send the unwrapped read to a replica. It sets a clean error packet, ends the request, and finishes the query. The failure path inside `fail_wait_wrap_finalize()` also sets `polardb_wait_disabled = true` so every later read this session uses the writer instead of an unwrapped replica read.
...
The backend sends four result sets for a wrapped read: one per `SET`, then the user result. The connection layer is the sole owner of dropping the `SET` results. In `PgSQL_Connection::handler()` (`lib/PgSQL_Connection.cpp:564-592`):

- While `polardb_query_wrap_state.has_pending()` and at the simple-query end state (`:564-565`):
  - a `SET` completing (`PGRES_COMMAND_OK` / `PGRES_EMPTY_QUERY`) → decrement the pending count, recycle the result buffer, null `query_result`, and fetch the next result (`:566-578`). The dropped `SET` result never reaches the client.
  - a `SET` that errored (`PGRES_FATAL_ERROR` / `PGRES_NONFATAL_ERROR` / `PGRES_BAD_RESPONSE`) → this is a wrapper `SET` failure, for example a strict-mode timeout surfacing as an ERROR. Account it via `polardb_account_wrapper_set_error()` and stop consuming so the error flows to the client (`:579-590`).
...
`polardb_handle_notice()` (`lib/PgSQL_PolarDB_Notices.cpp:97`):
- ignores any notice whose structured `PG_DIAG_MESSAGE_DETAIL` field is not the marker `polar_proxy_lsn_wait_timeout` (`include/PgSQL_PolarDB.h:94`). ProxySQL matches the structured marker, never human-readable text, so user SQL cannot fake a timeout (`lib/PgSQL_PolarDB_Notices.cpp:107-113`).
- confirms the consistency wait is active (`:128-142`).
- accounts the timeout via `polardb_account_wait_timeout("notice")` (`:149`).
- builds a fresh NoticeResponse packet and queues it with `enqueue_pending_notice()` (`:168-198`).
...
- if a required RFQ LSN was missing: set the corresponding missing-LSN latch and increment the missing-LSN counter.

### 5.5 S5 — cleanup

After process_result, at the `__cleanup` label of `RequestEnd()`, the per-query PolarDB state is reset (`lib/PgSQL_Session.cpp:6113-6128`):
- `record_wait_latency()` runs first, so a completed wait charges its elapsed time to the latency counter; it is a no-op if no wait was active (`:6122`).
- the per-query reader target, wait state, wrapper buffer, and
  session-to-connection bridge fields are reset.
- any pending notices that were not forwarded (error paths) are freed (`:6128`).
...
There is a parallel reset on session reset / RESET / DISCARD at `lib/PgSQL_Session.cpp:383-394`, and on the wrapper-set-failure path (`rc == -1`) at `:3724-3742`, where `record_wait_latency()` is called but the timeout counter is deliberately not charged (only marker-confirmed timeouts are charged, comment at `:3727-3730`).
...
| `polardb_wait_disabled` | writer-fallback safety latch | session (cleared on RESET) |
...
|----------------|--------------|
| S1 drop | 3 `SET` results dropped; only the `SELECT` result kept (`Connection.cpp:564-578`) |
...
| Response stage | What happens |
|----------------|--------------|
| S1 drop | 3 `SET` results dropped (`Connection.cpp:566-578`) |
| S2 notice | the replica gives up after 1000 ms and serves possibly-stale rows **with a WARNING**. The WARNING carries `PG_DIAG_MESSAGE_DETAIL = polar_proxy_lsn_wait_timeout`. `polardb_handle_notice()` matches the marker (`Notices.cpp:107-113`), accounts the timeout via `polardb_account_wait_timeout("notice")` (`Notices.cpp:149`), and queues a NoticeResponse (`Notices.cpp:168-198`) |
...
**Result:** the client gets rows plus one WARNING. The data may be stale. The timeout is accounted exactly once — even though the same backend event could be observed twice — because the first accounting zeroes `wait_started_at_us` and the de-dup guard then suppresses any second count (`Wrap.cpp:267-274`, `:287-296`).
...
The client sends one packet with more than one statement, for example `SELECT 1; SELECT 2;`. collect scans for a semicolon (only when replica-eligible) and sets `is_multi_statement = true` (`Flow.cpp:85-96`).
...
| Stage | What happens |
|-------|--------------|
| gate | `PgHGM->status.polardb_active` is false → the whole `if` block at `Session.cpp:2543` is skipped |
| (rest) | normal ProxySQL query-rule routing applies; no collect/plan/execute runs |
| process_result | the process_result hook is guarded by `is_polardb_enabled`, which is false, so it does not run (`Session.cpp:6102`) |

The only PolarDB code that executes is the cheap atomic gate read and the
...
If a PolarDB pair exists but the current query targets a non-PolarDB hostgroup, collect returns early (`Flow.cpp:53-57`) and the caller's `is_polar_hg` guard stops the pipeline (`Session.cpp:2555`). If the user manually routed the query (`replica_eligible` unset and a destination set), the manual-mode check skips the pipeline (`Session.cpp:2546-2551`).
...
**Result:** the query behaves exactly as upstream ProxySQL. When the feature is compiled out (`POLARDB_PROXY=0`) there is not even the gate read; the binary is byte-equivalent to upstream by design (see [02-BUILD-TOGGLE-AND-LIBPQ.md](02-BUILD-TOGGLE-AND-LIBPQ.md)).
...
- **Safe writer fallback never silently breaks RYW.** Every uncertain case routes to the writer: malformed packet (`Flow.cpp:408-415`), the `polardb_wait_disabled` latch (`Flow.cpp:422-429`), and a wrap-build failure that returns an error to the client instead of running the unwrapped read on a replica (`Session.cpp:3596-3604`).
