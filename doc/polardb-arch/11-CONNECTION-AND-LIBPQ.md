...
This document covers the parts of the PolarDB LSN-only read-your-writes (RYW) feature that live on the **backend-connection** layer, the class `PgSQL_Connection`. A `PgSQL_Connection` is one TCP connection from ProxySQL to a PostgreSQL/PolarDB backend server, wrapped around a libpq `PGconn`.
...
All of this code is gated behind the compile flag `POLARDB_PROXY`. With `POLARDB_PROXY=0` every block below compiles out and the connection layer behaves exactly like upstream ProxySQL (see [02-BUILD-TOGGLE-AND-LIBPQ.md](02-BUILD-TOGGLE-AND-LIBPQ.md)).

**Status:** stable. Everything in this document is implemented and active in the LSN-only branch.
...
client read  ─►  route pipeline (Hook 2)
                 ...
                 sets polardb_dispatch_wrapper_*        (ASYNC_IDLE, Connection.cpp:1721)
                                                              │
                                                        query_start():
                                                        begin(n) consumer  (1440)
                                                              │
                                                        backend runs 3 SETs + query
                                                              │
                                                        WIRE filter (Hook 3):
                                                        drop 3 SET results,        (564)
                 ...
                 polardb_process_result reads ───────────────► get_polardb_lsn()  (1349)
                 the RFQ LSN                             (PQhasLSN / PQgetLSN)
```

Two things connect at the conninfo step (Hook 1): the conninfo may gain profile-driven PolarDB proxy startup parameters, and the session is marked PolarDB-enabled so the result-processing stage will run later. A successful startup only proves the server accepted the parameters; RFQ LSN is confirmed later when result RFQs actually carry LSN.
...
| `uint64_t get_polardb_lsn()` | `PgSQL_Connection.h:820` | `PgSQL_Connection.cpp:1349` | Return the LSN libpq cached from the last RFQ. Pure accessor; issues no SQL query. Returns 0 if the RFQ carried no LSN or there is no connection. |
...
|--------|---------|----------------|
| `static bool polardb_is_lsn_wait_timeout_result(const PGresult* result)` | `PgSQL_Connection.cpp:26` | Return true when the result's structured detail field `PG_DIAG_MESSAGE_DETAIL` equals the marker `POLARDB_LSN_WAIT_TIMEOUT_DETAIL`. This is how a strict-mode wait timeout is identified without matching human-readable text. |
| `static void polardb_account_wrapper_set_error(PgSQL_Connection* conn, const PGresult* result, const char* msg)` | `PgSQL_Connection.cpp:31` | When a wrapper SET (or the user query after wrapper consumption) errors, decide whether it is a PolarDB wait timeout and, if so, account it; then mark the wrap-state state as failed so consumption stops. |

`polardb_handle_notice()` itself is **declared** in this file (`PgSQL_Connection.cpp:24`) but **defined** in `lib/PgSQL_PolarDB_Notices.cpp:97` (see [08-WAIT-TIMEOUT-AND-NOTICES.md](08-WAIT-TIMEOUT-AND-NOTICES.md)).

### 3.5 Persistent connection-local state

| Member | Type | Declared | Purpose |
|--------|------|----------|---------|
| `dispatch_state` | `PolarDB_Query_DispatchState { uint32_t wrapper_stmts; PolarDB_Query_WrapperKind wrapper_kind; }` | `PgSQL_Connection.h:669` (struct `:661`) | Per-dispatch handoff: how many wrapper SET results to skip and which wrapper kind. Snapshotted from the session at dispatch, consumed by `query_start()`. |
| `polardb_query_wrap_state` | `PolarDB_Query_WrapState` | `PgSQL_Connection.h:776` (struct `:746`) | Per-query countdown of wrapper result sets. Drops the leading SETs and forwards only the user result. `was_wrapped`/`stmt_failed` are sticky (survive consumption). |
...
The check and the two actions are at `PgSQL_Connection.cpp:1204-1221`:

| Step | What it does | file:line |
|------|--------------|-----------|
| `is_polar = parent && PgHGM->is_polardb_hostgroup(parent->myhgc->hid)` | Ask the HostGroups Manager if this HG is a PolarDB HG. | `PgSQL_Connection.cpp:1210` |
...
| `myds->sess->polardb_config.is_polardb_enabled = true` | Mark the session PolarDB-enabled so the response-path result-processing stage runs. | `PgSQL_Connection.cpp:1219` |
...
```
PgSQL_Connection.cpp:1228-1231
...
|---|---|
| `v15` | `_polar_proxy_client_host=<host>`, `_polar_proxy_client_port=<port>`, `_polar_proxy_send_lsn=true` |
| `legacy` | `_polar_origin_client_ip=<host>`, `_polar_origin_client_port=<port>`, `_polar_send_lsn=true` |
| `off` | no PolarDB proxy startup parameters |

Notes:

- These must be **top-level** connection parameters, not inside the libpq `options` block, so PolarDB's `ProcessStartupPacket()` reads them. The conninfo `options` block is closed (the trailing `'`) at `PgSQL_Connection.cpp:1202`, before these parameters are appended at `:1204+`.
- The function is called only for PolarDB hostgroups. A vanilla PostgreSQL backend would reject an unknown startup option, so guarding the call keeps non-PolarDB connections safe (`PgSQL_Connection.cpp:1276-1278`).
...
- The request bit names are `REQUEST_RFQ_LSN`, `REQUEST_RFQ_CSN`, and `REQUEST_RFQ_XID`. Current `v15` and `legacy` profiles request `REQUEST_RFQ_LSN`; `off` requests none.

### 4.3 `polardb_init_connection_tracking` — turn on libpq LSN parsing
...
```
PgSQL_Connection.cpp:453-457
  #if POLARDB_PROXY
      // Turn on PolarDB WAL-LSN reporting on this backend ...
      polardb_init_connection_tracking();
  #endif
```

The function itself does two guards then one call:

| Step | Behavior | file:line |
|------|----------|-----------|
| Guard: live connection | Return early if `pgsql_conn` is null or `PQstatus(...) != CONNECTION_OK`. | `PgSQL_Connection.cpp:1326-1328` |
| Guard: startup profile | Return early unless `polardb_startup_profile.requests(REQUEST_RFQ_LSN)`. | `PgSQL_Connection.cpp` |
| Action | `PQsetPolarSendLSN(pgsql_conn, 1)` — turn on the libpq flag that makes `getReadyForQuery()` parse the appended LSN when present. | `PgSQL_Connection.cpp` |

Citation precision: `PQsetPolarSendLSN` is **defined** in the libpq patch at `polardb_libpq.patch` (function definition; see [02-BUILD-TOGGLE-AND-LIBPQ.md](02-BUILD-TOGGLE-AND-LIBPQ.md)). The **call site** in this tree is `PgSQL_Connection.cpp:1335`. (Some earlier notes used the libpq function line 1325 and the call line 1335; in this file the method `polardb_init_connection_tracking` begins at `PgSQL_Connection.cpp:1325` and the `PQsetPolarSendLSN` call is at `:1335`.)

This is still not capability confirmation. It only tells patched libpq to parse an RFQ LSN if the backend later sends one. Missing RFQ LSN is handled in result processing and routing policy; the connection is not closed solely because startup was requested but result RFQ LSN has not yet been confirmed.
...
|------|--------|-----------|
| Pooled (got a pooled connection) | `polardb_config.is_polardb_enabled = true` | `PgSQL_Session.cpp:5697` |
| Fresh (no pooled connection; will create new) | `polardb_config.is_polardb_enabled = true` | `PgSQL_Session.cpp:5711` |
...
```
PgSQL_Connection.cpp:1721-1728  (inside the ASYNC_IDLE case of async_query)
...
- Only **simple queries** are wrapped. The `!extended_query_info` guard at `PgSQL_Connection.cpp:1722` skips the snapshot for extended-protocol (Parse/Bind/Execute) queries, which are never wrapped.
...
```
PgSQL_Connection.cpp:1440-1453
...
`begin(n, kind)` (defined inline in `PgSQL_Connection.h:760`) sets `was_wrapped = (n>0)`, `stmt_total = n`, `stmt_pending = n`, and the wrapper kind. For an ordinary, non-wrapped query, `wrapper_stmts` is 0, so `begin()` is never called and `polardb_query_wrap_state` stays empty — the result path then skips the filter entirely.

`dispatch_state` is also reset on connection cleanup (`PgSQL_Connection.cpp:2921`), and `polardb_query_wrap_state` is cleared on the same cleanup (`PgSQL_Connection.cpp:2922`), so a connection returned to the pool carries no stale wrapper state.
...
|---------------|---------|-------------------|-----------|
| `PGRES_COMMAND_OK` or `PGRES_EMPTY_QUERY` | A wrapper SET finished cleanly. | Decrement `stmt_pending`, recycle the result buffer into `query_result_reuse`, null `query_result`, and immediately fetch the next result. The dropped SET result never reaches the client. | `PgSQL_Connection.cpp:566-578` |
| `PGRES_FATAL_ERROR`, `PGRES_NONFATAL_ERROR`, or `PGRES_BAD_RESPONSE` | A wrapper SET itself errored (for example, a strict-mode wait timeout surfaces as an ERROR). | Call `polardb_account_wrapper_set_error()` (accounts a confirmed timeout, marks the connection WrapState failed), then **stop** consuming so the error flows to the client through the normal path below. | `PgSQL_Connection.cpp:579-590` |

The gate `has_pending()` (`stmt_pending > 0`, defined in `PgSQL_Connection.h:753`) plus the `fetch_result_end_st == ASYNC_QUERY_END` check at `PgSQL_Connection.cpp:565` mean the filter only runs while wrapper SETs are still pending and only for simple queries (the only thing that can be wrapped). After `stmt_pending` reaches 0, the next result — the user query — falls through to the normal result-forwarding code and reaches the client.
...
| Site | When | file:line |
|------|------|-----------|
| `ASYNC_USE_RESULT_START`, error present before fetching | The fetch start sees an error already set; account it before ending the result. | `PgSQL_Connection.cpp:521-522` |
| `ASYNC_USE_RESULT_CONT`, FATAL / bad-response default branch | A FATAL/non-fatal/bad result after the error is set from the result; account it. | `PgSQL_Connection.cpp:700-702` |

There is also a trace-only block in the `ASYNC_QUERY_START`/`ASYNC_QUERY_CONT` early-error paths (`PgSQL_Connection.cpp:482-488` and `:503-510`); those only log (under `POLARDB_DEBUG`) and do not account anything.

### 5.4 `polardb_account_wrapper_set_error` — the accounting decision

Defined at `PgSQL_Connection.cpp:31`. It is the single place that decides whether a wrapper-path error is a PolarDB wait timeout to charge. Its logic, step by step:

| Step | Behavior | file:line |
|------|----------|-----------|
| Guard: was wrapped | If there is no connection or the connection's result was not wrapped (`!was_wrapped`), do nothing. | `PgSQL_Connection.cpp:34-42` |
| Guard: wait active | If there is no session or the session's wait is not active (`!polardb_wait_active()`), just mark the connection WrapState failed and return (no accounting). | `PgSQL_Connection.cpp:44-52` |
| Classify | `is_lsn_timeout = polardb_is_lsn_wait_timeout_result(result)` — check the structured marker. | `PgSQL_Connection.cpp:54` |
| Classify for retry | If `is_lsn_timeout`, set the query wait state's timeout flag. The session failure path uses this flag to decide whether the original read can be retried on the writer. | `PgSQL_Connection.cpp:69` |
| Account | If `is_lsn_timeout`, call `sess->polardb_account_wait_timeout("result-error")`. | `PgSQL_Connection.cpp:70-71` |
| Mark failed | If still consuming a wrapper SET, mark the connection WrapState failed (stops consumption). | `PgSQL_Connection.cpp:73-76` |

The comment at `PgSQL_Connection.cpp:63-68` explains the "wait active" gate: in strict mode the timeout can surface after all wrapper SET results were already consumed (the wait was started by `SET polar_xact_split_wait_lsn`, then the backend raises ERROR before running the user SELECT). Charging only the structured-marker event while the wait is active prevents an ordinary user-query error — whose text might coincidentally look like a timeout — from being miscounted. The timeout flag is separate from accounting because accounting is idempotent and may consume the latency timer before the session's `rc == -1` branch runs. The counters touched by `polardb_account_wait_timeout()` and by retry are documented in [08-WAIT-TIMEOUT-AND-NOTICES.md](08-WAIT-TIMEOUT-AND-NOTICES.md) and [12-THREADVARS-AND-OBSERVABILITY.md](12-THREADVARS-AND-OBSERVABILITY.md).
...
libpq's notice receiver is `notice_handler_cb()`, registered per query in `query_start()` (`PgSQL_Connection.cpp:1456`). Its body, at `PgSQL_Connection.cpp:2535`:

```
PgSQL_Connection.cpp:2540-2563
...
The key is the `if/else` on `query_result`: at most one of "store inline" and "queue in `pending_notices`" stores the notice for the client. `polardb_handle_notice()` adds to `pending_notices` only when (a) the notice carries the timeout marker and (b) the wait is active and the connection WrapState is a consistency wait (`is_consistency_wait()`); otherwise it leaves the notice to the generic path (`lib/PgSQL_PolarDB_Notices.cpp:128-142`). So a normal notice that already went inline is never re-queued, and a marker notice that arrived with no `query_result` is captured exactly once. The session-side queue (`pending_notices`, `enqueue_pending_notice`, `clear_pending_notices`) and the forward-once flush are covered in [08-WAIT-TIMEOUT-AND-NOTICES.md](08-WAIT-TIMEOUT-AND-NOTICES.md) and [10-SESSION-INTEGRATION.md](10-SESSION-INTEGRATION.md).

`add_notice` itself is a method of `PgSQL_Query_Result` (declared `include/PgSQL_Protocol.h:497`); it returns the byte count so the connection can update its received-bytes statistic.
...
```
PgSQL_Connection.cpp:1349-1360
...
|----------|--------|-----------|
| No round-trip | Calls only the native libpq accessors `PQhasLSN()` / `PQgetLSN()`. Safe on the hot request/result-processing path. | `PgSQL_Connection.cpp:1355-1356` |
| Returns 0 when absent | No connection, connection not OK, or the RFQ carried no LSN -> returns 0. | `PgSQL_Connection.cpp:1350-1352`, `:1359` |
| Enabled by | The per-connection LSN parsing must have been turned on by `PQsetPolarSendLSN(conn,1)` in `polardb_init_connection_tracking()` (`PgSQL_Connection.cpp:1335`). Without it, `PQhasLSN()` is always false. | — |

The caller is `polardb_process_result()` (defined in `lib/PgSQL_PolarDB_Flow.cpp`), reached from `PgSQL_Session::RequestEnd()` on the success path. Everything the result-processing stage does with the returned LSN — advancing session write/observed LSN state, maintaining missing-LSN latches, and refreshing the per-server cache — is documented in [09-PUBLISH-AND-WRITE-TRACKING.md](09-PUBLISH-AND-WRITE-TRACKING.md). The deliberate "RFQ-only, never SQL" rule keeps all SQL-based LSN probing in the monitor; see [05-MONITOR-AND-HGM-LSN-STATE.md](05-MONITOR-AND-HGM-LSN-STATE.md).

---

## 7. The libpq patch surface this layer depends on

The connection layer depends on the PolarDB-specific libpq patch: three new functions plus accepted startup parameters. None exist in stock libpq; they are all added by the patch `deps/postgresql/polardb_libpq.patch`, which is applied only when `POLARDB_PROXY=1` (see [02-BUILD-TOGGLE-AND-LIBPQ.md](02-BUILD-TOGGLE-AND-LIBPQ.md) for the full patch review). The connection layer is the only consumer of these in the LSN-only tree.

| libpq item | Kind | Used by connection layer at | What it does |
|------------|------|------------------------------|--------------|
| `PQsetPolarSendLSN(PGconn*, int enable)` | function | `PgSQL_Connection.cpp:1335` | Turn on the runtime flag that makes libpq's RFQ parser look for an appended LSN. |
| `PQhasLSN(const PGconn*)` | function | `PgSQL_Connection.cpp:1355` | Was an LSN present in the last RFQ? |
| `PQgetLSN(const PGconn*)` | function | `PgSQL_Connection.cpp:1356` | Return the LSN captured from the last RFQ (0 if none). |
...
The patch accepts more parameters than this tree emits. This implementation emits the LSN and client-identity parameters for the selected proxy protocol, and leaves cancel-routing / SSL metadata unused.
...
The patch registers the legacy names, the v15 names, cancel metadata, SSL metadata, and the three LSN functions (`PQgetLSN`, `PQhasLSN`, `PQsetPolarSendLSN`).
...
- **The connection layer is the sole owner of `WrapState` filtering.** The session and client never see the dropped SET results. If a future change moved this filtering elsewhere, the count must stay consistent with the count copied through `dispatch_state` (`PgSQL_Connection.cpp:1721-1728`).
- **The filter is bounded by `stmt_pending` and the simple-query end state.** The guard `has_pending() && fetch_result_end_st == ASYNC_QUERY_END` (`PgSQL_Connection.cpp:564-565`) means a stray extra result after `stmt_pending` reaches 0 is **not** dropped — it would be forwarded to the client. The header comment (`PgSQL_Connection.h:702-704`) documents this: a `SET` appended *after* the user query is not supported because there is no trailing-skip counter. The wrap point only ever prepends, so this is safe today.
...
- **Enable is one-way and multi-sourced.** `is_polardb_enabled` is set in three places (conninfo `:1219`, pooled `:5697`, fresh `:5711`) and never cleared. A pooled connection cannot bypass the result-processing stage, because the session enable is set on attach even though the connection-layer connect path did not run.
- **The notice `if/else` is essential to correctness.** The inline `add_notice` and the `pending_notices` queue are mutually exclusive for a given notice, which is why the client never sees a duplicate NoticeResponse. Removing the `else`/fall-through, or returning early when `query_result` is null, would drop wrapped-wait timeout notices (`PgSQL_Connection.cpp:2540-2563`).
- **Structured marker, not text.** Timeout classification uses `PG_DIAG_MESSAGE_DETAIL == "polar_proxy_lsn_wait_timeout"` (`PgSQL_Connection.cpp:26-29`, marker `include/PgSQL_PolarDB.h:94-95`), so user SQL cannot fake a timeout. This is a correctness property, not a convenience.
- **Both wrap-state and dispatch state are cleared on cleanup.** `PgSQL_Connection.cpp:2921-2922` resets `dispatch_state` and clears `polardb_query_wrap_state` so a connection returning to the pool carries no stale wrapper state into the next session.
...
| Connection-local `polardb_startup_profile` with `REQUEST_RFQ_LSN` gating | implemented |
| Session enable on fresh connect, pooled attach, and fresh attach | implemented (`:1219`, `PgSQL_Session.cpp:5697`, `:5711`) |
| `polardb_init_connection_tracking` / `PQsetPolarSendLSN` | implemented (`PgSQL_Connection.cpp:1325`, `:1335`) |
| `get_polardb_lsn` RFQ-only accessor | implemented (`PgSQL_Connection.cpp:1349`) |
| Wrap-state filter (drop N SETs, forward user result) | implemented (`PgSQL_Connection.cpp:564-592`) |
| Wrapper-error accounting via structured marker | implemented (`PgSQL_Connection.cpp:31`, `:70`) |
| Notice receiver fall-through to `polardb_handle_notice` | implemented (`PgSQL_Connection.cpp:2562`) |
| `dispatch_state` session→connection bridge | implemented (`PgSQL_Connection.cpp:1721-1728`, `:1440-1453`) |
...
- **Trailing-skip support.** The wrap-state filter has no counter for a SET *after* the user query (`PgSQL_Connection.h:702-704`); not needed today because the wrap only prepends.
- **No FORCE_PRIMARY / writer-fallback counter.** When a read is forced to the writer, nothing is counted on the connection layer or elsewhere; an operator cannot see how often reads bypass replicas (a known observability gap, see [12-THREADVARS-AND-OBSERVABILITY.md](12-THREADVARS-AND-OBSERVABILITY.md)).
...
```mermaid
flowchart TD
  A[connect_start builds conninfo] --> B{parent HG is PolarDB?<br/>PgHGM->is_polardb_hostgroup<br/>Connection.cpp:1210}
  ...
  F --> SE[set session is_polardb_enabled=true<br/>Connection.cpp:1219]
  G --> SE
  X --> SE
  C --> PQ[PQconnectStart]
  SE --> PQ
  PQ --> I{connect OK?}
  I -- yes --> J[polardb_init_connection_tracking<br/>Connection.cpp:456]
  J --> K{live conn AND<br/>REQUEST_RFQ_LSN requested?}
  K -- yes --> L[PQsetPolarSendLSN conn,1<br/>Connection.cpp:1335]
  K -- no --> M[no-op]
```
...
```mermaid
flowchart TD
  S[session finalize_wait_timeout_injection<br/>sets polardb_query.dispatch_wrapper_stmts=3] --> A{ASYNC_IDLE, simple query?<br/>Connection.cpp:1722}
  A -- yes --> B[snapshot into dispatch_state<br/>zero session fields<br/>Connection.cpp:1724-1727]
  A -- no/extended --> Z[no snapshot]
  B --> C[query_start: begin n,kind<br/>Connection.cpp:1442]
  C --> D[backend runs 3 SETs + user query]
  D --> E{result while has_pending AND<br/>simple-query end? Connection.cpp:564}
  E -- COMMAND_OK / EMPTY_QUERY --> F[stmt_pending--, recycle buffer,<br/>fetch next Connection.cpp:566-578]
  F --> E
  E -- error status --> G[polardb_account_wrapper_set_error<br/>stop consuming Connection.cpp:579-590]
...
```mermaid
flowchart TD
  N[backend NOTICE/WARNING] --> CB[notice_handler_cb<br/>Connection.cpp:2535]
  CB --> Q{query_result != null?<br/>Connection.cpp:2540}
  Q -- yes --> I[query_result->add_notice<br/>store inline Connection.cpp:2543]
  Q -- no --> D[debug log only, do NOT return<br/>Connection.cpp:2552]
  I --> P[polardb_handle_notice<br/>Connection.cpp:2562]
  D --> P
  P --> M{marker AND wait active AND<br/>is_consistency_wait?<br/>Notices.cpp:128-142}
...
flowchart TD
  ...
  B --> C[get_polardb_lsn<br/>Connection.cpp:1349]
  C --> D{conn OK?}
  D -- no --> Z[return 0]
  D -- yes --> E{PQhasLSN?<br/>Connection.cpp:1355}
  E -- yes --> F[return PQgetLSN<br/>Connection.cpp:1356]
  E -- no --> Z
```
