...
A **Log Sequence Number (LSN)** is a 64-bit position in PostgreSQL's write-ahead log. A larger LSN means a more recent point in the write history. In code the type is `XLogRecPtr`, a `typedef` for `uint64_t`, with the constant `InvalidXLogRecPtr = 0` standing for "no LSN" (`include/PgSQL_PolarDB.h:101-103`).
...
### 3.3 The wait gate on the replica
...
When ProxySQL sends `SET polar_xact_split_wait_lsn = '<target>'` to a replica before a read, the replica blocks until its replay LSN passes that target, and only then runs the read. **This SET is the actual correctness gate of the feature.** The replica does the waiting; ProxySQL only supplies the target.
...
The wrapper is built by `build_wrapped_wait_query()` (`lib/PgSQL_PolarDB_Wrap.cpp:136`). It is installed exactly once, at one point in the code (covered in Section 7.2 and in `07-QUERY-WRAPPING.md`).
...
| Transaction split | offload a read inside an open transaction to a replica | `19-FUTURE-TXN-SPLIT-DESIGN.md` |
| Reader-failure recovery | retry/forward logic when a replica read fails | `20-FUTURE-READER-FAILURE-RETRY-DESIGN.md` |
...
The transaction-split source file (`PgSQL_PolarDB_Split.cpp`) exists only in the full implementation, not in this tree. A narrow reader-failure foundation does exist here as `lib/PgSQL_PolarDB_Failure.cpp` (autocommit wait-wrapped read retry on the writer); the general reader-failure model (RETRY / FORWARD / writer-loss handling inside a transaction) remains future work. This tree is a strict subset.

### 4.3 The reasoning behind the slice

There are four reasons to ship the LSN path first.

1. **It is about one third of the code.** The LSN path is much smaller than the full feature. Seven dedicated source files implement it (listed in `POLARDB_ARCHITECTURE.md`). Transaction split alone adds a large state machine in a separate file in the full implementation. Less code is easier to review correctly and to land upstream.

2. **It is the foundation everything else builds on.** CSN, transaction split, and reader-failure recovery all reuse the same four pipeline stages and the same wait-gate idea. Getting collect/plan/execute/process_result right first means the later features extend a proven base rather than redesigning it. The structs were deliberately designed to leave room for the later features (for example, the wait snapshot keeps a spare merge point for CSN; see `03-TYPES-AND-ENUMS.md`).

3. **Autocommit-only keeps the rules simple.** This version only sends a read to a replica when it is **not** inside an explicit transaction. A read inside an open `BEGIN ... COMMIT` block always goes to the writer. That removes a whole class of hard cases (importing transaction IDs, mid-transaction visibility) that transaction split would need. The routing rules that enforce this are in `06-ROUTING-PIPELINE.md`.
...
The code follows five rules. Each rule has a one-line statement, the reason, and where it shows up in code. Full proofs are in `14-INVARIANTS-AND-FAILURE-MODES.md`.
...
**Where:** the routing plan returns `FORCE_PRIMARY` (route to the writer) for every uncertain case — an explicit transaction, a multi-statement read, an extended-protocol read with a known or unknown LSN target, the lag cap exceeded, a `route=primary` hint, or `PRIMARY_ONLY` mode. Each case carries a recorded reason. The full list with file:line is in `06-ROUTING-PIPELINE.md`. If building the wait wrapper fails, the failure handler sets a session latch (`polardb_wait_disabled = true`) so later reads use the writer, and the wrap call site returns an error for the current read rather than silently sending an unwrapped read to a replica (`lib/PgSQL_Session.cpp:3607`).
...
**Where:** `finalize_wait_timeout_injection()` (`lib/PgSQL_PolarDB_Wrap.cpp:303`) is the only function that builds the wrapper. It is called from one site: the `ASYNC_IDLE` block in `PgSQL_Session::handler()` at `lib/PgSQL_Session.cpp:3607`, after the backend connection exists and just before the query runs. The earlier routing stage only saves the intent (which reader, which target LSN); it does not build the wrapper. Detail is in `07-QUERY-WRAPPING.md`.
...
**Why:** ProxySQL produces both PolarDB and non-PolarDB builds from one codebase. Gating the whole feature behind one flag means a non-PolarDB build carries none of this code and behaves exactly like upstream. It also keeps the feature easy to reason about as a self-contained addition.

**Where:** every PolarDB declaration and every core call site is wrapped in `#if POLARDB_PROXY`. The flag defaults on (`POLARDB_PROXY ?= 1` at `Makefile:149`) and becomes the `-DPOLARDB_PROXY` define in each build stage. A dedicated translation unit `lib/PgSQL_PolarDB_Stubs.cpp` is reserved for link-time no-op stubs; today it is intentionally empty (`#if !POLARDB_PROXY` with only a comment, `lib/PgSQL_PolarDB_Stubs.cpp:28`) because every seam is already guarded. The libpq RFQ-LSN patch is applied only when `POLARDB_PROXY=1`, so a `POLARDB_PROXY=0` build links vanilla libpq. Full detail is in `02-BUILD-TOGGLE-AND-LIBPQ.md`.

> **Note on byte-equivalence.** "`POLARDB_PROXY=0` is byte-equivalent to upstream" is the in-code design contract stated in the stub TU comment (`lib/PgSQL_PolarDB_Stubs.cpp:10-23`), confirmed by reading the guards. It has not been re-checked here by a build-and-diff.
...
|---|---|---|---|
| collect | `polardb_collect` (`lib/PgSQL_PolarDB_Flow.cpp:233`) | snapshot all routing inputs into a `PolarDB_Query_RouteCtx` (`include/PgSQL_PolarDB.h:1550`); also repairs stale session LSN state when the writer group or epoch changed | `06-ROUTING-PIPELINE.md` |
| plan | `polardb_plan` (`lib/PgSQL_PolarDB_Flow.cpp:410`) | turn that context into a `PolarDB_Query_RoutePlan` (`include/PgSQL_PolarDB.h:1692`) with action PASSTHROUGH / FORCE_PRIMARY / REPLICA_WITH_WAIT | `06-ROUTING-PIPELINE.md` |
| execute | `polardb_execute` (`lib/PgSQL_PolarDB_Flow.cpp:658`) | apply the decision, returning a `PolarDB_Query_ExecuteResult` (`include/PgSQL_PolarDB.h:1918`); prepare the wait state | `06-ROUTING-PIPELINE.md` |
...
```
HOOK route: polardb_active condition          (lib/PgSQL_Session.cpp:2563)
        |
        v
   collect  (Flow.cpp:233)  -->  plan (Flow.cpp:410)  -->  execute (Flow.cpp:658)
        |
        v
HOOK wrap: finalize_wait_timeout_injection   (Session.cpp:3607 -> Wrap.cpp:303)
...
### 7.1 The route hook (gate into the pipeline)

The pipeline runs only when at least one PolarDB hostgroup is configured. The route hook is in `PgSQL_Session::get_pkts_from_client()`, gated by the atomic master flag `PgHGM->status.polardb_active`:

- The gate check is at `lib/PgSQL_Session.cpp:2563`.
- `polardb_collect(...)` is called at `lib/PgSQL_Session.cpp:2563`.
- `polardb_plan(...)` is called at `lib/PgSQL_Session.cpp:2566`.
- `polardb_execute(...)` is called at `lib/PgSQL_Session.cpp:2569`, and its result overwrites `current_hostgroup`.
...
### 7.2 The wrap hook (the gate, single wrap point)

When the plan chose `REPLICA_WITH_WAIT`, the wait wrapper is built once at `ASYNC_IDLE` by `finalize_wait_timeout_injection()` (call site `lib/PgSQL_Session.cpp:3607`; function `lib/PgSQL_PolarDB_Wrap.cpp:303`). On failure it returns an error and ends the request instead of sending an unwrapped read to a replica. This is the gate that turns the routing intent into the actual SET-wrapped query on the wire.

### 7.3 The process_result hook (write tracking)

On the success response path, `RequestEnd()` calls `polardb_process_result()` (call site `lib/PgSQL_Session.cpp`; function `lib/PgSQL_PolarDB_`), guarded by `polardb_config.is_polardb_enabled`. This reads the writer's RFQ LSN and advances the session write LSN. This is RFQ-only (Section 5.3).
...
| POLARDB_PROXY | The compile-time macro / make variable (default 1) that gates the entire PolarDB feature. Set via `-DPOLARDB_PROXY` (`lib/Makefile`, `src/Makefile`; `Makefile:149`). When 0, all PolarDB declarations and call sites compile out and the binary is byte-equivalent to upstream ProxySQL (design contract, `lib/PgSQL_PolarDB_Stubs.cpp:10-23`). |
| LSN (Log Sequence Number) | A 64-bit position in PostgreSQL's write-ahead log (WAL); larger = more recent. Carried as `XLogRecPtr` (typedef `uint64_t`, `InvalidXLogRecPtr=0`, `include/PgSQL_PolarDB.h:101-103`). A replica that has replayed up to LSN X can serve any read whose data was committed at or before X. |
...
| HGM (HostGroups Manager) | `PgSQL_HostGroups_Manager`: the class that owns backend topology and the global `PgHGM->status` struct holding all PolarDB atomic counters and the `polardb_active` gate. |
| polardb_active | An `atomic<bool>` on `HGM::status` (`include/PgSQL_HostGroups_Manager.h:788`, set at `lib/PgSQL_HostGroups_Manager.cpp:2095` from whether any PolarDB hostgroup is configured). The cheap master gate: every HGM PolarDB accessor and the route pipeline (`lib/PgSQL_Session.cpp:2563`) check it first to fail safe when no PolarDB hostgroup is configured. |
| consistency mode | The per-query routing policy, resolved by precedence session override > per-HG > global. Values in this version: OFF=0 (no PolarDB routing, query rules decide), SESSION_LSN=1 (wait on the session's write LSN before a replica read), PRIMARY_ONLY=3 (force all reads to the writer). Enum `PolarDB_ConsistencyMode` at `include/PgSQL_PolarDB.h:728`; default global value `'off'`. The value `'default'` / -1 means "not set, defer to the lower tier". **FUTURE** adds CSN and global modes. |
...
| result processing | The response-path stage `polardb_process_result` (`lib/PgSQL_PolarDB_Flow.cpp`) that reads RFQ payloads, advances session write/observed LSN state, maintains missing-LSN latches, and refreshes the per-server LSN cache. RFQ-only; no SQL. |
...
| polar_xact_split_wait_lsn | The PolarDB backend GUC that is the actual RYW wait gate: on a replica the backend blocks until its replay LSN passes this target before answering. Appended by `append_polar_wait_set` (`include/PgSQL_PolarDB.h:1460`); appends nothing for a zero/NONE target. |
| polar_consistency_mode | The PolarDB GUC choosing timeout behavior on the replica: `best_effort` (return stale data + WARNING) or `strict` (raise ERROR). Set by ProxySQL from the wait-timeout-mode knob; cached and rebuilt only on change (`build_polar_consistency_mode_set`, `lib/PgSQL_PolarDB_Wrap.cpp:95`). |
...
| PASSTHROUGH | Plan action meaning "do not override routing". With target -1 the caller leaves the hostgroup chosen by query rules; with target >=0 the caller pins that HG (e.g. the writer when not replica-eligible). |
...
| polardb_wait_disabled | A session latch set true when a wait wrapper cannot be built; while true, replica-with-wait reads fall back to the writer so RYW is never silently broken. Cleared on RESET. |
...
| stats_pgsql_global | The ProxySQL admin table (name, value rows) exposing PostgreSQL global status counters, including all PolarDB counters. PolarDB exposes **26 stat counters plus 1 internal `polardb_active` gate**; the gate is not a counter. The same counters are also exported as `proxysql_polardb_*_total` Prometheus metrics. |
...
| transaction split | **FUTURE.** Running a read-only statement inside an open `BEGIN..COMMIT` transaction (after a write) on a replica by importing the writer's transaction IDs, while the write transaction stays on the primary. Lives in the full implementation only; absent here. See `19-FUTURE-TXN-SPLIT-DESIGN.md`. |
...
| the RYW guarantee stated precisely and the safe fallback proofs | `14-INVARIANTS-AND-FAILURE-MODES.md` |
...
flowchart TD
    ...
    N["all gated by POLARDB_PROXY (compile-time isolation, 5.5)"]
```
...
```mermaid
flowchart LR
    H1["HOOK route: polardb_active condition\nSession.cpp:2543"] --> S1[collect Flow.cpp:233]
    S1 --> S2[plan Flow.cpp:410]
    S2 --> S3[execute Flow.cpp:658]
    S3 --> H2["HOOK wrap: finalize_wait_timeout_injection\nSession.cpp:3607 -> Wrap.cpp:303"]
