...
```
  ...
  (1) Monitor readonly check  ── polardb_update_server_lsn ─► per-server cache:    │   ┌──► get_MyConn_polardb_reader
      (PgSQL_Monitor.cpp:1946)                      │   PgSQL_SrvC          │   │    (lag-cap safety)
                                                    │   .polardb_current_lsn│   │
...
The list of read-only servers is built by a JOIN of `pgsql_servers` with `pgsql_replication_hostgroups`, and that SELECT carries the `check_type` column (`PgSQL_Monitor.cpp:553-558`):
...
Because the JOIN matches both `writer_hostgroup` and `reader_hostgroup`, every server in a PolarDB pair (writer **and** readers) appears in this list and inherits the pair's `check_type`. The value is copied onto the per-server task descriptor `mon_srv_t.check_type` (`PgSQL_Monitor.cpp:337`, set at `:519`).

### 3.2 The PolarDB health query

When building the read-only task's SQL, `get_task_query()` checks the per-server `check_type`. If it equals `"polardb"` (case-insensitive), it sends the 3-column PolarDB query; otherwise it sends the plain PostgreSQL `pg_is_in_recovery()` query (`PgSQL_Monitor.cpp:895-904`).

The PolarDB query is `POLARDB_CHECK_WITH_LSN_QUERY` (`PgSQL_Monitor.cpp:58-62`):
...
The read-only result handler runs on the async-query path. When `POLARDB_PROXY` is set and the result has 3 or more columns, it parses the PolarDB health columns (`PgSQL_Monitor.cpp:730-759`):
...
`parse_polardb_full_health_check()` (`lib/PgSQL_PolarDB.cpp:75-81`) fills a `PolarDB_HealthCheck` value (`include/PgSQL_PolarDB.h:256`) by delegating to three static helpers in `PolarDB_Protocol`: `parse_node_type`, `parse_is_available`, and `parse_lsn_string`. The parsed LSN, the read-only flag, and the availability flag are stored in a `readonly_res_t` result object that survives until the action phase (`PgSQL_Monitor.cpp:354-360`):
...
- LSN text must parse as a PostgreSQL WAL LSN such as `0/4201DD30`. Invalid LSN
  ...
  not counted as invalid; the normal LSN update gate ignores zero.
...
After a successful read-only check, `perf_readonly_actions()` runs the normal read-only bookkeeping, then the PolarDB block (`PgSQL_Monitor.cpp:1939-1962`):

```c
#if POLARDB_PROXY
    if (polardb_should_update_monitor_lsn(
...
1. **LSN update.** The gate predicate `polardb_should_update_monitor_lsn()` (`include/PgSQL_PolarDB.h:564-567`) returns true only when the `pgsql-polardb_monitor_lsn_updates` knob is on **and** the observed LSN is greater than 0. It then calls `polardb_update_server_lsn()` (section 5). The monitor counter `PolarDB_LSN_Updates_From_Monitor` is bumped **only when `polardb_update_server_lsn()` returns true**, that is, only when the LSN actually advanced. See section 8.
...
|----------|-------|--------|
| Which task carries the LSN | the read-only check (`task_type_t::readonly`) | `PgSQL_Monitor.cpp:729-759` |
...
| Which servers | every server in a `check_type='polardb'` replication pair (writer and readers) | JOIN at `PgSQL_Monitor.cpp:553-558` |
| On/off switch | `pgsql-polardb_monitor_lsn_updates` (default `true`) | `PgSQL_Thread.cpp:1128` (default); gate at `include/PgSQL_PolarDB.h:564-567` predicate |
| Skipped when | the observed LSN is 0, or the knob is off, or no PolarDB HG is configured | predicate `include/PgSQL_PolarDB.h:564-567`; `polardb_update_server_lsn` gate `HGM.cpp:4544` |
...
|---|---|---|
| Where | `PgSQL_Monitor.cpp:1946` | `PgSQL_PolarDB_` (the result-processing stage) |
...
Every backend server is a `PgSQL_SrvC` object. Under `POLARDB_PROXY` it carries two atomic LSN fields (`include/PgSQL_HostGroups_Manager.h:214-229`):
...
- The header comment block (`include/PgSQL_HostGroups_Manager.h:215-228`) states two things: these two fields are read without a lock by the reader acquisition and by the lag/freshness helpers; and millisecond replica lag is a deferred PolarDB TODO (lines 220-222), because PgSQL does not currently produce a real value for `aws_aurora_current_lag_us`. See sections 7 and 9.
...
| Step | Code | Detail |
|------|------|--------|
| 1. Fast gate | `HGM.cpp:4544` | If `status.polardb_active` is false, return `false` immediately (no PolarDB HG configured). |
| 2. Null guard | `HGM.cpp:4545` | If `hostname == nullptr` or the direct `PgSQL_SrvC*` is missing, return `false`. |
...
| 5. Stale/offline guard | `HGM.cpp` | Host/port monitor samples skip `OFFLINE_HARD` server objects. Direct RFQ skips when the request writer hostgroup/epoch is missing, resolves to a different writer hostgroup, or no longer equals the current writer epoch. It does not read mutable server status in the result hot path. |
| 6. Compute advance | `HGM.cpp:4559` | `advanced = (lsn > current polardb_current_lsn)`. |
...
| 8. Mirror primary | `HGM.cpp` | Host/port monitor samples mirror only from configured writer HGCs. Direct RFQ advances the snapshot's shared writer primary-LSN cell after the current writer epoch check. Both mirrors are monotonic (`lsn > current_primary`). |
| 9. Return | `HGM.cpp:4578` | Return `advanced` (true only if the per-server LSN moved forward). |
| 10. No match | `HGM.cpp:4583-4584` | If no server matched, unlock and return `false`. |
...
- **The primary mirror is monotonic within a writer epoch.** Step 7 never lowers `polardb_primary_lsn`; an older observation cannot pull the cached primary position backward. When the writer identity set changes, HGM bumps the writer epoch and resets the primary mirror plus the affected per-server LSN caches to avoid carrying old-timeline values forward.
...
polardb_update_server_lsn(host, port, lsn)
        ...
        ▼  wrlock()
   for each HGC / each srv:
        if srv.address==host && srv.port==port:
            if srv is OFFLINE_HARD:
                continue
            advanced = lsn > srv.polardb_current_lsn
            CAS-max srv.polardb_current_lsn = lsn  ──► (per-server cache)
            if writer HGC configured and srv is active and lsn > primary_mirror:
                writer.repl_config.polardb_primary_lsn = lsn   ──► (primary mirror)
            wrunlock(); return advanced
   wrunlock(); return false   (no match)
```
...
The HGM keeps three private containers describing the PolarDB topology (`include/PgSQL_HostGroups_Manager.h:1066-1068`):
...
All three are cleared and rebuilt during a config commit, inside `PgSQL_HostGroups_Manager` while holding the write lock. For each row of `pgsql_replication_hostgroups` whose `check_type` is `"polardb"` (`HGM.cpp:1824-1831`):
...
After the loop, the master gate is set from whether the set is non-empty (`HGM.cpp:1864`):
...
The per-pair policy is cached on the **writer** hostgroup container `PgSQL_HGC::repl_config` (an anonymous struct, `include/PgSQL_HostGroups_Manager.h:284-294`), populated in the same commit loop (`HGM.cpp:1836-1843`):
...
| `polardb_writer_identity_initialized` | bool (=false) | **runtime**, set true once the first identity is recorded | epoch reset detection (guards the first identity sample) |
...
`polardb_consistency_mode_from_string()` maps the schema word to an int: `default` or unknown -> `-1` (means "not set, defer to lower tier"), `off` -> 0, `lsn` -> 1, `primary` -> 3 (defined `include/PgSQL_PolarDB.h:751-767`, called at `HGM.cpp:2059`). The string-to-int and 3-tier resolution details are in [04-ADMIN-SCHEMA-AND-CONFIG.md](04-ADMIN-SCHEMA-AND-CONFIG.md).
...
The routing pipeline asks for a hostgroup's PolarDB config through `get_polardb_hg_config()` (`HGM.cpp:4964-4977`). It returns a `PolarDB_HG_Config` from the generation-cached topology snapshot. The config bundles `is_polardb_hostgroup`, the resolved writer/reader HG ids, the policy (consistency mode, lag cap, wait timeout), and the shared `primary_lsn` / `writer_epoch` cells. Logic:

1. Fast gate: if `polardb_active` is false, return the default (not configured) (`HGM.cpp:4966`).
2. Take the thread-local generation-cached topology snapshot via `get_polardb_topology_snapshot_cached()`; if there is none, return the default (`HGM.cpp:4968-4969`).
3. Look up `snapshot->by_hostgroup.find(hostgroup_id)` and return the stored `PolarDB_HG_Config` (`HGM.cpp:4972-4976`).

The reader->writer mapping and the policy copy happen once at commit time when the snapshot is built: the commit loop stores the *same* `PolarDB_HG_Config` (writer-side policy) under BOTH the writer id and the reader id (`HGM.cpp:2091-2106`). So a lookup by either id yields the writer-side policy, and the maps `polardb_reader_to_writer_` / `polardb_writer_to_reader_` and the writer HGC's `repl_config` are NOT consulted by this accessor at query time.

This means a query that arrives pointed at the **reader** HG still resolves the correct writer-side policy. `get_polardb_hg_policy()` is a thin wrapper that returns just the `.policy` part (`HGM.cpp:4979`).
...
- **HGM:** `PgSQL_HostGroups_Manager::is_polardb_hostgroup()` (`HGM.cpp:4929-4941`): fast gate on `polardb_active`, then a lock-free `get_polardb_topology_snapshot_cached()` lookup returning `snapshot->by_hostgroup.count(id) > 0` (no `wrlock()`). The `polardb_hostgroups_` set is only populated at config-commit time (`HGM.cpp:2035, 2065-2066`); the steady-state membership test reads the generation-cached snapshot.
...
Every public PolarDB HGM accessor checks `status.polardb_active` (an atomic load, no lock) before doing anything, and returns a "not configured" answer when it is false. This is the cheap master gate: when no PolarDB pair is configured, every accessor returns a safe "not configured" answer and never touches the shared maps.

| Accessor | Gate site | Returns when gate off |
|----------|-----------|-----------------------|
| `is_polardb_hostgroup` | `HGM.cpp:4932` | `false` |
| `get_writer_hostgroup_for_reader` | `HGM.cpp:4453` | `-1` |
| `get_reader_hostgroup_for_writer` | `HGM.cpp:4462` | `-1` |
| `get_polardb_hg_config` | `HGM.cpp:4475` | default (not configured) |
| `polardb_update_server_lsn` | `HGM.cpp:4544` | `false` |
| `get_polardb_primary_lsn` | `HGM.cpp:4588` | `0` |

The route pipeline itself also checks the same gate before doing any PolarDB work (`PgSQL_Session.cpp:2543`). See [06-ROUTING-PIPELINE.md](06-ROUTING-PIPELINE.md).

---

## 7. The byte-lag cap and the freshness gate

### 7.1 What the lag cap is (and is not)

`max_lag_bytes` is an **optional safety bound** on `primary_lsn - reader_lsn` (the replica's lag in WAL bytes). It is **not** the RYW correctness gate. The correctness gate is the wait `SET` the proxy prepends to the read; the lag cap is a separate guard that refuses a replica that has fallen too far behind, regardless of the wait. When the cap is exceeded the read uses the writer instead of that replica.
...
if (now_us < updated_at_us)  return true;   // clock-skew guard
return (now_us - updated_at_us) <= freshness_ms * 1000;
```

So a cached LSN counts only if it was set within `pgsql-polardb_lsn_freshness_ms` (default 5000 ms). A stale entry can still be part of the ordinary wait-capable set when only `consistency_target_lsn` is present, because the backend wait remains the correctness gate. Under an enabled byte-lag cap, stale or missing reader LSNs are hard skips; if every candidate is rejected this way, the query uses the writer.

### 7.4 `get_polardb_primary_lsn()`

`get_polardb_primary_lsn()` returns the writer's mirrored primary LSN through the generation-cached topology snapshot: fast gate on `polardb_active`, snapshot map lookup by writer HG, then `primary_lsn->load()`. It does **not** take the HGM lock and it does not retain a `PgSQL_HGC` pointer. There is **no freshness check on the primary side** — within a writer epoch the primary mirror is treated as monotonic and authoritative for "latest write position". If it is still 0 (no write or monitor sample yet, or immediately after writer-epoch reset), the cap check rejects the reader and uses the writer (section 7.2).
...
Five of the 26 exported PolarDB stat counters are fed from the monitor/HGM LSN
state area. `polardb_active` is an internal boolean gate, not a counter. Storage
...
|------------------------|----------------|--------|------------------|------------------|
| `PolarDB_Server_LSN_Updates_From_RFQ` | `PgSQL_PolarDB_Flow.cpp:516` | every finished query whose RFQ carried an LSN and passed the direct current-group/current-epoch HGM update gate (writes and reads) | none | grows under normal traffic; flat-zero with traffic means the RFQ-LSN path is not active or RFQs are rejected as stale/missing writer group/epoch data |
| `PolarDB_LSN_Updates_From_Monitor` | `PgSQL_Monitor.cpp:1947` | monitor read-only checks where the LSN **advanced** (`polardb_update_server_lsn()` returned true) | none | grows = healthy background refresh; flat-zero = monitor updates off, no advance seen, or no PolarDB HG |
...
Declaration lines: `polardb_server_lsn_updates_from_rfq`, `polardb_lsn_updates_from_monitor`, `polardb_monitor_health_invalid_role`, `polardb_monitor_health_invalid_values`, `polardb_lsn_stale_count`, and the gate `polardb_active` are in `include/PgSQL_HostGroups_Manager.h`.
...
This counter is exported and active for the byte-lag gate: reader acquisition increments it when an enabled `max_lag_bytes` check cannot trust the primary mirror or a reader LSN sample. The separate `pgsql-polardb_lag_ms` runtime variable still accepts only `0`, has no millisecond-lag producer, and remains deferred in [15-LIMITATIONS-AND-ROADMAP.md](15-LIMITATIONS-AND-ROADMAP.md).
...
| `polardb_active` gate | `PgHGM->status` | `std::atomic<bool>` |
...
- The `polardb_active` gate is checked first in every accessor and in the route pipeline, so when no PolarDB pair is configured the whole subsystem returns "not configured" without touching the maps.

### 9.4 Reader acquisition note (`get_MyConn_polardb_reader`)

`get_MyConn_polardb_reader()` (`HGM.cpp:4602`) is the reader-picking function. Two facts matter for this document:
...
| Item | Status | Notes |
|------|--------|-------|
| Monitor LSN feed (read-only check, 3-column query, parse, cache update) | implemented | `PgSQL_Monitor.cpp:553-558, 730-759, 1939-1962` |
...
| Availability shun from `polar_is_available()` | implemented | `PgSQL_Monitor.cpp:1955-1961` |
| Per-server LSN cache (`polardb_current_lsn`, `lsn_updated_at`) | implemented | `HGM.h:227-228` |
...
| Writer epoch (`polardb_writer_epoch`) | implemented | bumps on sorted active writer identity-set change; collect/process_result scopes stale session targets/latches |
| Topology maps + `is_polardb_hostgroup` + `polardb_active` gate | implemented | `HGM.cpp:1810-1864`, `:4441` |
| Byte-lag cap + freshness gate | implemented (safety-only, off by default) | `Flow.cpp:145-178`, `HGM.cpp:4512-4541` |
| `PolarDB_Server_LSN_Updates_From_RFQ`, `PolarDB_LSN_Updates_From_Monitor` | implemented | `Flow.cpp:516`, `Monitor.cpp:1947` |
| **Millisecond replica-lag cap (`pgsql-polardb_lag_ms`)** | **deferred / inert** | no PgSQL producer; default 0; `HGM.cpp:4436-4438` |
...
- `HGM.cpp:4436` — `pgsql-polardb_lag_ms` is deferred; no real millisecond replica-lag producer; use byte lag and freshness instead.
...
- `HGM.cpp:4637`, `:4661-4668` — deferred millisecond-lag safety filter; keep visibly guarded until a producer exists; suggested `estimated_catchup_ms = byte_lag / recent_replay_bytes_per_ms`.
...
- `PgSQL_Monitor.cpp:1998` — (non-PolarDB, nearby) override-replication is hardcoded to false, to be revisited.
...
flowchart LR
    subgraph Producers
      MON["Monitor read-only check<br/>PgSQL_Monitor.cpp:1946"]
      RFQ["Query RFQ result update<br/>PgSQL_PolarDB_"]
      CFG["Config commit<br/>HGM.cpp:1810-1864"]
    end
...
flowchart TD
    ...
    C -- yes --> Z1
    C -- no --> D["wrlock()"]
    D --> E["walk HGCs / servers"]
    E --> F{"address==host<br/>and port==port?"}
    F -- no match --> G["wrunlock(); return false"]
    F -- match --> H["advanced = lsn > current"]
    H --> I["store polardb_current_lsn = lsn<br/>store lsn_updated_at = now"]
    I --> J{"writer configured<br/>and lsn > primary_mirror?"}
    J -- yes --> K["primary_mirror = lsn (monotonic)"]
    J -- no --> L["wrunlock()"]
    K --> L
    L --> M["return advanced"]
```
...
```mermaid
flowchart TD
    A["read-only task<br/>task_type_t::readonly"] --> B{"check_type == 'polardb'?<br/>Monitor.cpp:900"}
