...
void ProxySQL_Admin::disk_upgrade_pgsql_replication_hostgroups() {
	...
	// PolarDB transaction-split config upgrade: V3_0_4 -> V3_0_5
	// (add stored txn_split_enabled, defaulting to disabled).
	rci = configdb->check_table_structure(
		const_cast<char*>("pgsql_replication_hostgroups"),
		const_cast<char*>(ADMIN_SQLITE_TABLE_PGSQL_REPLICATION_HOSTGROUPS_V3_0_4)
	);

	if (rci) {
		proxy_warning("Detected version 3.0.4 of table 'pgsql_replication_hostgroups'\n");
		proxy_warning("ONLINE UPGRADE of table 'pgsql_replication_hostgroups' in progress\n");
		configdb->execute("DROP TABLE IF EXISTS pgsql_replication_hostgroups_v304");
		configdb->execute("ALTER TABLE pgsql_replication_hostgroups RENAME TO pgsql_replication_hostgroups_v304");
		configdb->build_table(
			const_cast<char*>("pgsql_replication_hostgroups"),
			const_cast<char*>(ADMIN_SQLITE_TABLE_PGSQL_REPLICATION_HOSTGROUPS),
			false
