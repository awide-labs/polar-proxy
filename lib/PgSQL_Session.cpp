...
static bool pgsql_digest_has_copy_statement(const char* digest_text) {
	const char* statement = digest_text;
	while (statement && *statement) {
		while (*statement &&
				std::isspace(static_cast<unsigned char>(*statement))) {
			statement++;
		}
		if (strncasecmp(statement, "COPY", 4) == 0) {
			const unsigned char next = static_cast<unsigned char>(statement[4]);
			if (next == '\0' || std::isspace(next) || next == '(') {
				return true;
			}
		}
...
static bool polardb_debug_reset_timeout_requested() {
	char buf[64] = {0};
	const char* fault_file = "POLARDB_DEBUG_READER_ACQUIRE_FAULT_FILE";
	return polardb_debug_read_fault_file(fault_file, buf, sizeof(buf)) &&
		strcmp(buf, "reset_timeout") == 0;
}

// Enter reset for a target-ready pooled reader, then force its timeout branch.
// The first check leaves the fault file intact; this call consumes it only
// after the session has reached RESETTING_CONNECTION_V2.
static bool polardb_debug_reset_timeout() {
	const char* fault_file = "POLARDB_DEBUG_READER_ACQUIRE_FAULT_FILE";
	if (!polardb_debug_reset_timeout_requested()) return false;
...
#if POLARDB_PROXY
#define POLARDB_QUERY_PARSER_COUNT(sess, name, value) do { \
		if ((sess) && (sess)->thread) { \
			POLARDB_PROFILE_THREAD_COUNT((sess)->thread, name, (value)); \
		} \
	} while (0)

#define POLARDB_QUERY_PARSER_COUNT_ONE(sess, name) \
	POLARDB_QUERY_PARSER_COUNT((sess), name, 1)
#endif // POLARDB_PROXY
...
#if POLARDB_PROXY
		POLARDB_QUERY_PARSER_COUNT_ONE(sess, query_parser_init);
		POLARDB_QUERY_PARSER_COUNT(sess, query_parser_init_bytes,
			QueryLength > 0 ? static_cast<unsigned long long>(QueryLength) : 0);
		if (pgsql_thread___query_digests) {
			POLARDB_QUERY_PARSER_COUNT_ONE(sess,
				query_parser_init_digest_enabled);
		}
		if (pgsql_thread___commands_stats) {
			POLARDB_QUERY_PARSER_COUNT_ONE(sess,
				query_parser_init_commands_enabled);
		}
#endif // POLARDB_PROXY
		query_parser_init();
		if (pgsql_thread___commands_stats)
...
#if POLARDB_PROXY
	POLARDB_QUERY_PARSER_COUNT_ONE(sess, query_parser_command_type);
#endif // POLARDB_PROXY
...
unsigned long long PgSQL_Query_Info::query_parser_update_counters() {
#if POLARDB_PROXY
	POLARDB_QUERY_PARSER_COUNT_ONE(sess, query_parser_update);
#endif // POLARDB_PROXY
	if (extended_query_info.stmt_info) {
		PgQueryCmd= extended_query_info.stmt_info->PgQueryCmd;
	}
	if (PgQueryCmd==PGSQL_QUERY___NONE) { // this means that it was never initialized
#if POLARDB_PROXY
		POLARDB_QUERY_PARSER_COUNT_ONE(sess, query_parser_update_skipped_none);
#endif // POLARDB_PROXY
		return 0;
	}
	if (PgQueryCmd==PGSQL_QUERY__UNINITIALIZED) { // this means that it was never initialized
#if POLARDB_PROXY
		POLARDB_QUERY_PARSER_COUNT_ONE(sess,
			query_parser_update_skipped_uninitialized);
#endif // POLARDB_PROXY
		return 0;
	}
#if POLARDB_PROXY
	if (QueryParserArgs.digest_text) {
		POLARDB_QUERY_PARSER_COUNT_ONE(sess,
			query_parser_update_with_digest);
	}
#endif // POLARDB_PROXY
...
#if POLARDB_PROXY
void PgSQL_Session::polardb_enter_reader_capacity_wait(
		uint64_t scope_hash, PolarDB_ReaderStatus reader_status,
		PgSQL_SrvC* reservation_server,
		std::shared_ptr<const void> reservation_server_snapshot,
		uint32_t reservation_profile_generation,
		const PolarDB_PoolKey* reservation_pool_key,
		PolarDB_PoolCapacityTarget target,
		const PolarDB_Query_ReaderPlan* reservation_plan,
		const PolarDB_WaitSpec* reservation_wait_spec) {
	PolarDB_ReaderCapacityWaitState& wait = polardb_reader_capacity_wait;
	if (!wait.active) {
		wait.active = true;
		wait.started_at_us = thread && thread->curtime
			? thread->curtime : monotonic_time();
		wait.scope_hash = scope_hash;
		POLARDB_THREAD_COUNT_ONE(thread, reader_capacity_wait_enter);
		if (target == PolarDB_PoolCapacityTarget::WRITER) {
			POLARDB_THREAD_COUNT_ONE(thread, writer_capacity_wait_enter);
		}
		if (thread) {
			thread->polardb_reader_capacity_wait_started(scope_hash);
		}
	} else if (wait.scope_hash != scope_hash && thread) {
		thread->polardb_reader_capacity_wait_scope_changed(
			wait.scope_hash, scope_hash);
	}
	wait.scope_hash = scope_hash;
	wait.target = target;
	wait.reservation_plan = reservation_plan
		? *reservation_plan : polardb_query.reader_plan;
	wait.reservation_wait_spec = reservation_wait_spec
		? *reservation_wait_spec : polardb_query.wait.spec;
	if (reader_status == PolarDB_ReaderStatus::READER_GROUP_BUSY &&
			reservation_server && reservation_server_snapshot && reservation_pool_key &&
			reservation_profile_generation != 0 && !reservation_pool_key->empty()) {
		wait.reservation_server = reservation_server;
		wait.reservation_server_snapshot = std::move(reservation_server_snapshot);
		wait.reservation_profile_generation = reservation_profile_generation;
		wait.reservation_pool_key = *reservation_pool_key;
	}
	wait.last_status = reader_status;
	wait.result_valid = true;
	wait.retry_admitted = false;
}

void PgSQL_Session::polardb_leave_reader_capacity_wait(
		PolarDB_ReaderStatus reader_status) {
	PolarDB_ReaderCapacityWaitState& wait = polardb_reader_capacity_wait;
	if (thread) {
		thread->polardb_cancel_reader_reservation(thread_session_id);
	}
	if (wait.active) {
		const uint64_t now = thread && thread->curtime
			? thread->curtime : monotonic_time();
		const uint64_t elapsed = now >= wait.started_at_us
			? now - wait.started_at_us : 0;
		POLARDB_THREAD_COUNT_ONE(thread, reader_capacity_wait_exit);
		POLARDB_THREAD_COUNT(thread, reader_capacity_wait_sum_us, elapsed);
		POLARDB_THREAD_MAX(thread, reader_capacity_wait_max_us, elapsed);
		if (wait.target == PolarDB_PoolCapacityTarget::WRITER) {
			POLARDB_THREAD_COUNT_ONE(thread, writer_capacity_wait_exit);
			POLARDB_THREAD_COUNT(thread, writer_capacity_wait_sum_us, elapsed);
		}
		if (elapsed <= 1000) {
			POLARDB_THREAD_COUNT_ONE(thread, reader_capacity_wait_le_1ms);
		} else if (elapsed <= 5000) {
			POLARDB_THREAD_COUNT_ONE(thread, reader_capacity_wait_le_5ms);
		} else if (elapsed <= 20000) {
			POLARDB_THREAD_COUNT_ONE(thread, reader_capacity_wait_le_20ms);
		} else if (elapsed <= 100000) {
			POLARDB_THREAD_COUNT_ONE(thread, reader_capacity_wait_le_100ms);
		} else if (elapsed <= 1000000) {
			POLARDB_THREAD_COUNT_ONE(thread, reader_capacity_wait_le_1s);
		} else {
			POLARDB_THREAD_COUNT_ONE(thread, reader_capacity_wait_gt_1s);
		}
		if (thread) {
			thread->polardb_reader_capacity_wait_finished(wait.scope_hash);
		}
	}
	wait.started_at_us = 0;
	wait.scope_hash = 0;
	wait.target = PolarDB_PoolCapacityTarget::READER;
	wait.reservation_plan.reset();
	wait.reservation_wait_spec.reset();
	wait.reservation_server = nullptr;
	wait.reservation_server_snapshot.reset();
	wait.reservation_profile_generation = 0;
	wait.reservation_pool_key = PolarDB_PoolKey{};
	wait.last_status = reader_status;
	wait.active = false;
	wait.retry_admitted = false;
	wait.result_valid = true;
...
void PgSQL_Session::reset() {
	...
	locked_on_hostgroup_and_all_variables_set = false;
#if POLARDB_PROXY
	if (polardb_txn_reader.backend || polardb_txn_reader.active()) {
		polardb_reset_txn_reader_request();
		polardb_release_txn_reader_backend(/*want_reuse=*/false);
	}
#endif // POLARDB_PROXY
	if (mybes) {
		reset_all_backends();
...
int PgSQL_Session::handler_again___status_RESETTING_CONNECTION() {
	...
	int rc;
#if POLARDB_PROXY && POLARDB_DEBUG
	if (polardb_query.wait_bypass_target != 0 &&
			polardb_debug_reset_timeout()) {
		POLARDB_TRACE(
			"PolarDB DEBUG: forcing reset-compatible reader timeout\n");
		rc = -2;
	} else
#endif // POLARDB_PROXY && POLARDB_DEBUG
	{
		rc = myconn->async_reset_session(myds->revents);
	}
	if (rc == 0) {
...
#if POLARDB_PROXY
#define POLARDB_APPLY_FAILURE_ACTION(action_, reset_retries_, forward_stmt_, passthrough_stmt_) do { \
	switch (action_) { \
...
bool PgSQL_Session::handler_again___verify_backend_user_db() {
	...
	bool needs_reset =
		myds->myconn->requires_RESETTING_CONNECTION(client_myds->myconn);
#if POLARDB_PROXY && POLARDB_DEBUG
	if (!needs_reset && polardb_query.wait_bypass_target != 0 &&
			polardb_debug_reset_timeout_requested()) {
		needs_reset = true;
	}
#endif // POLARDB_PROXY && POLARDB_DEBUG
	if (needs_reset) {
...
bool PgSQL_Session::handler_again___status_CONNECTING_SERVER(int* _rc) {
	...
	if (mybe->server_myds->max_connect_time) {
		...
		if (thread->curtime >= mybe->server_myds->max_connect_time) {
			...
			if (polardb_reader_capacity_wait.active) {
				if (polardb_reader_capacity_wait.target ==
						PolarDB_PoolCapacityTarget::WRITER) {
					const PolarDB_ReaderStatus wait_status =
						polardb_reader_capacity_wait.last_status;
					polardb_leave_reader_capacity_wait(wait_status);
				} else {
					PgSQL_Data_Stream* reader_myds = mybe->server_myds;
					const int writer_hg =
						polardb_reader_capacity_wait.reservation_plan.
							fallback_writer_hg;
					...
					if (fallback == PolarDB_ReadFallbackAction::PRIMARY &&
							polardb_redirect_to_writer(
								writer_hg,
								"replica capacity deadline reached")) {
						NEXT_IMMEDIATE_NEW(CONNECTING_SERVER);
					}
	...
	if (mybe->server_myds->myconn == NULL) {
		if (handler___client_DSS_QUERY_SENT___server_DSS_NOT_INITIALIZED__get_connection()) {
			NEXT_IMMEDIATE_NEW(WAITING_CLIENT_DATA);
		}
	}
	if (mybe->server_myds->myconn == NULL) {
	...
	if (mybe->server_myds->myconn == NULL) {
...
#endif // POLARDB_PROXY
		{
			pause_until = thread->curtime +
				pgsql_thread___connect_retries_delay * 1000;
		}
		*_rc = 1;
		return false;
	}
		...
		case 0:
#if POLARDB_PROXY
			if (myconn->polardb_startup_settings_set &&
					!PgHGM->polardb_reader_connection_is_current(
						myconn, mybe->hostgroup_id)) {
				const int writer_hg =
					polardb_query.reader_plan.fallback_writer_hg;
				const PolarDB_ReadFallbackAction fallback =
					polardb_read_fallback_action_from_int(
						polardb_query.reader_plan.read_fallback_action);
				POLARDB_THREAD_COUNT_ONE(
					thread, reader_pool_retry_after_config_change);
				POLARDB_TRACE(
					"PolarDB startup settings changed while connecting "
					"reader_hg=%d writer_hg=%d; rejecting backend\n",
					mybe->hostgroup_id, writer_hg);
				myds->destroy_MySQL_Connection_From_Pool(false);
				if (fallback == PolarDB_ReadFallbackAction::PRIMARY &&
						polardb_redirect_to_writer(
						writer_hg, "reader startup settings changed")) {
				...
				const char* errmsg =
					fallback == PolarDB_ReadFallbackAction::PRIMARY
						? "PolarDB reader configuration changed while connecting "
						  "and no primary fallback is available"
						: "PolarDB reader configuration changed while connecting "
						  "and the selected policy does not permit primary fallback";
				if (client_myds) {
					client_myds->myprot.generate_error_packet(
						true, true, errmsg,
						PGSQL_ERROR_CODES::
...
#if POLARDB_PROXY
				retry_conn = polardb_redirect_wait_retry_to_writer(
					retry_conn, "reset-compatible reader failure");
#endif // POLARDB_PROXY
				myds->destroy_MySQL_Connection_From_Pool(false);
				myds->fd = 0;
				if (retry_conn) {
...
#if POLARDB_PROXY
				retry_conn = polardb_redirect_wait_retry_to_writer(
					retry_conn, "reset-compatible reader timeout");
#endif // POLARDB_PROXY
				myds->destroy_MySQL_Connection_From_Pool(false);
				myds->fd = 0;
				if (retry_conn) {
...
#if POLARDB_PROXY
		retry_conn = polardb_redirect_wait_retry_to_writer(
			retry_conn, "cached-reader offline retry");
#endif // POLARDB_PROXY
		if (transaction_state_manager) {
			transaction_state_manager->reset_state();
		}
...
#if POLARDB_PROXY
	retry_conn = polardb_redirect_wait_retry_to_writer(
		retry_conn, "cached-reader connection retry");
#endif // POLARDB_PROXY
...
#if POLARDB_PROXY
		retry_conn = polardb_redirect_wait_retry_to_writer(
			retry_conn, "cached-reader server-error retry");
#endif // POLARDB_PROXY
...
int PgSQL_Session::RunQuery(PgSQL_Data_Stream* myds, PgSQL_Connection* myconn) {
	...
	int rc = 0;
#if POLARDB_PROXY && POLARDB_DEBUG
	static std::atomic<int> _rq_count{0};
	int _rq = _rq_count.fetch_add(1, std::memory_order_relaxed) + 1;
	POLARDB_TRACE("[RQ%03d.000] ======== RunQuery ENTRY ======== status=%d\n", _rq, (int)status);
	POLARDB_TRACE("[RQ%03d.001] myds=%p DSS=%d myconn=%p async_state=%d\n",
		_rq, (void*)myds, myds ? myds->DSS : -1, (void*)myconn,
		myconn ? (int)myconn->async_state_machine : -1);
	if (myds) {
		POLARDB_TRACE("[RQ%03d.002] real_query_ptr=%p real_query_len=%u\n",
			_rq, (void*)myds->pgsql_real_query.QueryPtr, myds->pgsql_real_query.QuerySize);
	}
#endif // POLARDB_PROXY && POLARDB_DEBUG
	switch (status) {
	case PROCESSING_QUERY:
#if POLARDB_PROXY && POLARDB_DEBUG
		POLARDB_TRACE("[RQ%03d.010] PROCESSING_QUERY: calling async_query\n", _rq);
#endif
		rc = myconn->async_query(myds->revents, myds->pgsql_real_query.QueryPtr, myds->pgsql_real_query.QuerySize);
#if POLARDB_PROXY && POLARDB_DEBUG
		POLARDB_TRACE("[RQ%03d.011] async_query returned rc=%d\n", _rq, rc);
#endif
		break;
	case PROCESSING_STMT_PREPARE:
		{
...
#if POLARDB_PROXY && POLARDB_DEBUG
	{
		static std::atomic<int> _sh_count{0};
		int _sh = _sh_count.fetch_add(1, std::memory_order_relaxed) + 1;
		POLARDB_TRACE("PolarDB [SH%03d] Session::handler() ENTRY status=%d client_myds=%p mybe=%p\n",
			_sh, status, (void*)client_myds, (void*)mybe);
	}
#endif // POLARDB_PROXY && POLARDB_DEBUG
	bool wrong_pass = false;
	bool in_pending_state = false;
#if POLARDB_PROXY
	auto polardb_output_coalesce_enabled = []() -> bool {
		return pgsql_thread___polardb_output_coalesce_bytes > 0 ||
			pgsql_thread___polardb_output_coalesce_packets > 0;
	};
	auto polardb_maybe_hold_streaming_output = [&](bool processing_extended_query) -> bool {
		if (!polardb_output_coalesce_enabled()) return false;
		if (processing_extended_query) return false;
		if (!client_myds || !client_myds->PSarrayOUT ||
			client_myds->PSarrayOUT->len == 0) {
			return false;
		}
		if (client_myds->active == 0 || client_myds->net_failure ||
			client_myds->queueOUT.head != client_myds->queueOUT.tail ||
			client_myds->queueOUT.partial != 0 ||
			client_myds->queueOUT.pkt.ptr != NULL ||
			client_myds->polardb_write_head_partial != 0) {
			if (PgHGM) {
				POLARDB_THREAD_COUNT_ONE(thread, output_coalesce_flush_backpressure);
			}
			return false;
		}
		const unsigned int buffered_data =
			client_myds->PSarrayOUT->len * PGSQL_RESULTSET_BUFLEN;
		if (buffered_data > overflow_safe_multiply<8, unsigned int>(
				pgsql_thread___threshold_resultset_size)) {
			// PgSQL_Connection stops fetching rows at this limit. Flush before
			// returning to poll so backend fetching can resume.
			if (PgHGM) {
				POLARDB_THREAD_COUNT_ONE(thread, output_coalesce_flush_backpressure);
			}
			return false;
		...
		const unsigned int packets = client_myds->PSarrayOUT->len;
		const unsigned int packet_budget = static_cast<unsigned int>(
			pgsql_thread___polardb_output_coalesce_packets);
		const unsigned int byte_budget = static_cast<unsigned int>(
			pgsql_thread___polardb_output_coalesce_bytes);
		const bool packet_budget_reached =
			packet_budget > 0 && packets >= packet_budget;
		bool byte_budget_reached = false;
		if (byte_budget > 0) {
			byte_budget_reached =
				client_myds->PSarrayOUT->total_size() >= byte_budget;
		}
		if (packet_budget_reached || byte_budget_reached) {
			if (PgHGM) {
				POLARDB_THREAD_COUNT_ONE(thread, output_coalesce_flush_budget);
			}
			return false;
		}

		if (PgHGM) {
			POLARDB_THREAD_COUNT_ONE(thread, output_coalesce_hold);
		}
		return true;
	};
#endif // POLARDB_PROXY
	...
	case PROCESSING_QUERY: {
		...
		if (mybe->server_myds->DSS == STATE_NOT_INITIALIZED) {
			...
			if (status == PROCESSING_QUERY || status == PROCESSING_STMT_PREPARE) {
				...
				if (digest_text) {
					if (!pgsql_digest_has_copy_statement(digest_text))
						run_match = false;
			...
			if (myconn->async_state_machine == ASYNC_IDLE) {
				...
				if (polardb_wait_active() &&
					...
					if (txn_wait_reader) {
						polardb_reconcile_txn_wait_read_end(
							"finalize_wait_failed", true);
					}
					client_myds->setDSS_STATE_QUERY_SENT_NET();
					...
					RequestEnd(txn_wait_reader ? NULL : myds, true);
					if (!txn_wait_reader) {
						finishQuery(myds, myconn, false);
					}
					goto __exit_DSS__STATE_NOT_INITIALIZED;
...
#if POLARDB_PROXY && POLARDB_DEBUG
			POLARDB_TRACE("[S] ======== SESSION RC HANDLING ======== rc=%d status=%d wait_active=%d wait_stage=%d\n",
				rc, (int)status, polardb_wait_active() ? 1 : 0, (int)polardb_query.wait.wait_stage);
#endif
			timespec endt;
			if (thread->variables.stats_time_backend_query) {
				clock_gettime(CLOCK_THREAD_CPUTIME_ID, &endt);
			...
			if (rc == 0) {
				...
				case PROCESSING_QUERY: {
#if POLARDB_PROXY
					const auto handle_polardb_result_failure =
						[&](const char* kind, PgSQL_Backend* failed_backend) {
							const PolarDB_RequestOutcome outcome =
								polardb_capture_request_outcome(failed_backend);
							const PolarDB_FailureAction action =
								polardb_handle_reader_failure(outcome);
							POLARDB_TRACE(
								"PolarDB FAILURE: result kind=%s action=%s "
								"backend_hg=%d timeout=%d reusable=%d counted=%d\n",
								kind, polardb_failure_action_name(action),
								outcome.backend_hg,
								outcome.timeout_error ? 1 : 0,
								outcome.reusable ? 1 : 0,
								outcome.timeout_already_accounted ? 1 : 0);
							return action;
						};
					...
					if (polardb_should_handle_wait_timeout_result(
						...
						polardb_result_failure_checked = true;
						myds->query_retries_on_failure = 0;
						const PolarDB_FailureAction polardb_failure_action =
							handle_polardb_result_failure(
								"wait_timeout", active_backend());
						POLARDB_APPLY_FAILURE_ACTION(polardb_failure_action,
							/*reset_retries=*/false,
							goto __exit_DSS__STATE_NOT_INITIALIZED,
							do {} while (0));
					}
					// A strict wait timeout is an ErrorResponse from the
					// prepended SET, not an rc=-1 connection failure. Handle
					// it before the replica error becomes client-visible.
					if (!polardb_result_failure_checked &&
						...
						const PolarDB_FailureAction polardb_failure_action =
							handle_polardb_result_failure(
								"wrapper", polardb_txn_reader.backend);
						POLARDB_APPLY_FAILURE_ACTION(polardb_failure_action,
							/*reset_retries=*/false,
							goto __exit_DSS__STATE_NOT_INITIALIZED,
							do {} while (0));
				...
				if (rc == -1) {
					...
					const PolarDB_RequestOutcome polardb_outcome =
						polardb_capture_request_outcome(active_backend());
					const PolarDB_FailureAction polardb_failure_action =
						polardb_handle_reader_failure(polardb_outcome);
					POLARDB_TRACE(
					...
					bool polardb_run_generic_error_path = false;
					POLARDB_APPLY_FAILURE_ACTION(polardb_failure_action,
						/*reset_retries=*/true,
						do {} while (0),
						polardb_run_generic_error_path = true);
#endif // POLARDB_PROXY
#if POLARDB_PROXY
...
#if POLARDB_PROXY
						if (polardb_maybe_hold_streaming_output(processing_extended_query)) {
							in_pending_state = true;
						}
#endif // POLARDB_PROXY
						break;
...
bool PgSQL_Session::handler___status_WAITING_CLIENT_DATA___STATE_SLEEP___handle_SET_command(const char* dig, bool* lock_hostgroup) {
	...
	// Remove trailing space, semicolon, and the simple-query packet terminator.
	// See issue#4380 for the original space/semicolon handling.
	while (!nq.empty() && (nq.back() == ' ' || nq.back() == ';' || nq.back() == '\0')) {
		nq.pop_back();
	}
#if POLARDB_PROXY
	auto parse_polardb_consistency_mode_value = [](std::string value, int* mode) -> bool {
		...
		*mode = polardb_consistency_mode_from_string(value.c_str(), -2);
		return *mode != -2;
	};
	...
	auto finish_polardb_internal_set = [&]() -> bool {
		client_myds->DSS = STATE_QUERY_SENT_NET;
	...
	auto handle_polardb_internal_set =
		...
		if (strcasecmp(pvar.c_str(), "polardb_consistency_mode") == 0) {
			...
			if (policy_error) {
				return reject_polardb_internal_policy(
					"proxysql.polardb_consistency_mode=global_lsn "
					"requires both missing-LSN and timeout actions "
					"to forbid degraded reader results");
			}
			polardb_set_session_consistency_mode(mode);
	...
	if (is_in_transaction()) {
		// SET inside an explicit transaction is backend-local state unless it
		// was consumed by the ProxySQL-owned branch above.
	...
	if (!session_default_isolation.empty()) {
		...
		if (session_default_update) {
			...
			// SET [SESSION] TRANSACTION is transaction-scoped, so it is
			// forwarded without changing PolarDB's session-default routing
			// condition.
...
#if POLARDB_PROXY
			// ProxySQL-internal PolarDB session controls are consumed by the proxy
...
bool PgSQL_Session::handler___status_WAITING_CLIENT_DATA___STATE_SLEEP___handle_RESET_command(const char* dig, bool* lock_hostgroup) {
...
#if POLARDB_PROXY
	// Check for RESET proxysql.* before stripping non-word characters; the dot is
	// part of the ProxySQL-internal namespace.
	{
		std::string raw = nq;
		RE2::GlobalReplace(&raw, "(?U)/\\*.*\\*/", "");
		RE2::GlobalReplace(&raw, "(?i)\\bRESET\\b", "");
		size_t start = raw.find_first_not_of(" \t\n\r;");
		if (start != std::string::npos && strncasecmp(raw.c_str() + start, "proxysql.", 9) == 0) {
			std::string pvar = raw.substr(start + 9);
			while (!pvar.empty() &&
				(pvar.back() == ' ' || pvar.back() == '\t' || pvar.back() == '\n' ||
				pvar.back() == '\r' || pvar.back() == ';' || pvar.back() == '\0')) {
				pvar.pop_back();
			}
	...
	if (strncasecmp(nq.c_str(), "ALL", 3) == 0) {
		...
		for (int idx : client_myds->myconn->dynamic_variables_idx) {
			[[maybe_unused]] const char* name = pgsql_tracked_variables[idx].set_variable_name;
			auto [value, hash] = client_myds->myconn->get_startup_parameter_and_hash((enum pgsql_variable_name)idx);
			uint32_t current_hash = pgsql_variables.client_get_hash(this, idx);
...
#if POLARDB_PROXY
	// Any RESET accepted above drops staged per-query wait/notice state.
	...
	if (reset_override) {
		reapply_user_attributes_after_reset();
	}
#endif // POLARDB_PROXY
	client_myds->DSS = STATE_QUERY_SENT_NET;
...
void PgSQL_Session::reapply_user_attributes_after_reset() {
	if (user_attributes && user_attributes[0]) {
		try {
			nlohmann::json attributes =
				nlohmann::json::parse(user_attributes);
			auto isolation =
				attributes.find("default-transaction_isolation");
			if (isolation != attributes.end()) {
				const std::string value = isolation->get<std::string>();
				(void)pgsql_variables.client_set_value(
					this, SQL_ISOLATION_LEVEL, value, true);
			}
		} catch (const std::exception& error) {
			proxy_warning(
				"Unable to reapply PostgreSQL user attributes after reset: %s\n",
				error.what());
		}
	}
#if POLARDB_PROXY
...
#if POLARDB_PROXY
	// A split-reader failure sends the rest of the transaction to the writer.
...
#if POLARDB_PROXY
		&& !polardb_query_cache_is_disabled()
#endif // POLARDB_PROXY
	) {
...
#endif
bool PgSQL_Session::handler___client_DSS_QUERY_SENT___server_DSS_NOT_INITIALIZED__get_connection() {
...
#if POLARDB_PROXY
	const bool polardb_data_path_enabled =
		polardb_query.data_path_enabled();
	const bool polardb_reader_pool_eligible =
		polardb_data_path_enabled &&
		polardb_query.reader_plan.fallback_writer_hg >= 0 &&
		polardb_query.reader_plan.read_target ==
			static_cast<int>(PolarDB_ReadTarget::REPLICA) &&
		polardb_query.reader_plan.fallback_writer_hg !=
			static_cast<int>(mybe->hostgroup_id) &&
		session_fast_forward == SESSION_FORWARD_TYPE_NONE &&
		!qpo->create_new_conn && trxid == 0 &&
		(qpo->max_lag_ms < 0 ||
			polardb_query.reader_wait_spec.has_wait());
	const bool polardb_writer_pool_eligible =
		polardb_query.profile_enabled &&
		polardb_query.request_writer_scope.valid() &&
		polardb_query.request_writer_scope.hg ==
			static_cast<int>(mybe->hostgroup_id) &&
		session_fast_forward == SESSION_FORWARD_TYPE_NONE &&
		!qpo->create_new_conn && trxid == 0 && qpo->max_lag_ms < 0;
	if (polardb_reader_capacity_wait.active &&
			((polardb_reader_capacity_wait.target ==
					PolarDB_PoolCapacityTarget::READER &&
				!polardb_reader_pool_eligible) ||
			 (polardb_reader_capacity_wait.target ==
					PolarDB_PoolCapacityTarget::WRITER &&
				!polardb_writer_pool_eligible))) {
		polardb_leave_reader_capacity_wait(
	...
	if (!polardb_reader_pool_eligible) {
			...
			if (mc && polardb_reader_capacity_wait.active &&
					polardb_reader_capacity_wait.target ==
						PolarDB_PoolCapacityTarget::WRITER) {
				polardb_leave_reader_capacity_wait(
...
//#define STRESSTESTPOOL_MEASURE
	...
	auto get_connection_for_current_backend = [&]() -> PgSQL_Connection* {
		if (trxid) {
			return PgHGM->get_MyConn_from_pool(mybe->hostgroup_id, this,
				(session_fast_forward || qpo->create_new_conn), uuid, trxid, -1);
		}
		return PgHGM->get_MyConn_from_pool(mybe->hostgroup_id, this,
			(session_fast_forward || qpo->create_new_conn), NULL, 0,
			(int)qpo->max_lag_ms);
	};

	if (mc == NULL) {
#if POLARDB_PROXY
		bool polardb_writer_acquisition_handled = false;
		if (polardb_writer_pool_eligible) {
			POLARDB_THREAD_COUNT_ONE(thread, writer_pool_acquire_attempt);
			PolarDB_Query_ReaderPlan writer_plan;
			writer_plan.fallback_writer_hg =
				static_cast<int>(mybe->hostgroup_id);
			writer_plan.read_target =
				static_cast<int>(PolarDB_ReadTarget::PRIMARY);
			PolarDB_WaitSpec no_wait;
			const bool confirm_writer_group_capacity =
				polardb_reader_capacity_wait.active &&
				polardb_reader_capacity_wait.target ==
					PolarDB_PoolCapacityTarget::WRITER &&
				polardb_reader_capacity_wait.retry_admitted;
			PolarDB_ReaderResult writer_result;
			if (thread) {
				writer_result.conn =
					thread->polardb_take_reader_reservation(this);
				if (writer_result.conn) {
					writer_result.srv = static_cast<PgSQL_SrvC*>(
						writer_result.conn->parent);
					writer_result.status = PolarDB_ReaderStatus::ACQUIRED;
				}
			}
			if (!writer_result.acquired()) {
				writer_result = PgHGM->polardb_acquire_reader_connection(
					mybe->hostgroup_id, this, writer_plan, no_wait, false,
					nullptr, -1, confirm_writer_group_capacity);
			}
			if (writer_result.acquired()) {
				POLARDB_THREAD_COUNT_ONE(thread, writer_pool_acquire_hit);
				mc = writer_result.conn;
				if (polardb_reader_capacity_wait.active) {
					polardb_leave_reader_capacity_wait(
						PolarDB_ReaderStatus::ACQUIRED);
				}
				polardb_writer_acquisition_handled = true;
				...
				if (writer_result.status ==
						PolarDB_ReaderStatus::READER_GROUP_BUSY) {
					POLARDB_THREAD_COUNT_ONE(
						thread, writer_pool_acquire_group_busy);
				} else {
					POLARDB_THREAD_COUNT_ONE(
						thread, writer_pool_acquire_busy);
				}
				polardb_enter_reader_capacity_wait(
					writer_result.retry_scope_hash, writer_result.status,
					writer_result.srv,
					std::move(writer_result.selected_server_snapshot),
					writer_result.reservation_profile_generation,
					&writer_result.reservation_pool_key,
					PolarDB_PoolCapacityTarget::WRITER,
					&writer_plan, &no_wait);
				polardb_writer_acquisition_handled = true;
			} else if (writer_result.status ==
				...
				polardb_writer_acquisition_handled = true;
			} else if (polardb_reader_capacity_wait.active &&
					polardb_reader_capacity_wait.target ==
						PolarDB_PoolCapacityTarget::WRITER) {
				polardb_leave_reader_capacity_wait(writer_result.status);
			}
		}
		bool polardb_reader_acquisition_handled = false;
		const PolarDB_WaitSpec& polardb_wait_spec =
			polardb_query.reader_wait_spec;
		if (!polardb_writer_acquisition_handled &&
				polardb_reader_pool_eligible) {
			const bool confirm_reader_group_capacity =
				polardb_reader_capacity_wait.active &&
				polardb_reader_capacity_wait.retry_admitted;
			PolarDB_ReaderResult reader_result;
			bool reader_pool_reservation_acquired = false;
			if (thread) {
				reader_result.conn = thread->polardb_take_reader_reservation(this);
				if (reader_result.conn) {
					reader_pool_reservation_acquired = true;
			...
			if (!reader_result.acquired()) {
				reader_result = PgHGM->polardb_acquire_reader_connection(
					mybe->hostgroup_id, this,
					polardb_query.reader_plan, polardb_wait_spec, false,
					nullptr, -1, confirm_reader_group_capacity);
			}
			if (reader_pool_reservation_acquired && polardb_wait_spec.has_wait() &&
					reader_result.srv) {
				...
				const PolarDB_ReaderLsnSample sample =
					reader_result.srv->polardb_sample_lsn(
				...
				reader_result.wait_bypass_allowed =
					sample.fresh &&
					sample.lsn >= polardb_wait_spec.target;
				polardb_count_reader_target_selection(
					thread, polardb_wait_spec.target,
					sample.lsn, sample.fresh,
					0, false);
#if POLARDB_PROFILE
				polardb_profile_note_reader_selection(
					polardb_wait_spec, reader_result);
			...
			if (reader_result.acquired() &&
				...
				polardb_leave_reader_capacity_wait(
					PolarDB_ReaderStatus::ACQUIRED);
			} else if (reader_result.status ==
					PolarDB_ReaderStatus::READER_BUSY ||
				reader_result.status ==
					PolarDB_ReaderStatus::READER_GROUP_BUSY) {
				polardb_enter_reader_capacity_wait(
					reader_result.retry_scope_hash, reader_result.status,
					reader_result.srv,
					std::move(reader_result.selected_server_snapshot),
					reader_result.reservation_profile_generation,
					&reader_result.reservation_pool_key,
					PolarDB_PoolCapacityTarget::READER,
					&polardb_query.reader_plan, &polardb_wait_spec);
			} else if (polardb_reader_capacity_wait.active) {
				polardb_leave_reader_capacity_wait(reader_result.status);
			}
			POLARDB_TRACE(
				"PolarDB consistency: polardb_acquire_reader_connection status=%s conn=%p "
				"target_lsn=%lu reader_hg=%d writer_hg=%d\n",
				polardb_reader_status_name(reader_result.status),
				reader_result.conn, (unsigned long)polardb_wait_spec.target,
				mybe->hostgroup_id, polardb_query.reader_plan.fallback_writer_hg);
			if (reader_result.acquired()) {
				// Activate wrapper state only if this concrete reader still needs
				// the wait. A target-ready reader dispatches the original packet.
				...
				// disconnect. Clearing it here silently restored the default
				// stale-with-warning action before the backend result arrived.
				polardb_reader_acquisition_handled = true;
			} else if (polardb_wait_spec.has_wait() &&
					reader_result.status == PolarDB_ReaderStatus::RFQ_UNAVAILABLE &&
					polardb_missing_lsn_action_from_int(
					polardb_query.reader_plan.missing_lsn_action) ==
						PolarDB_MissingLsnAction::WARNING &&
					polardb_query.reader_plan.allow_reader_without_target) {
				// The configured action allows a reader even though RFQ cannot
				// enforce the target. Queue a client-visible warning and retry
				// reader acquisition without the wait.
				...
				polardb_enqueue_degraded_rfq_notice(
					...
					reader_hg, writer_hg);
				POLARDB_TRACE(
					"PolarDB consistency: RFQ unavailable; "
					"retrying reader without wait target (reader_hg=%d writer_hg=%d)\n",
					reader_hg, writer_hg);
				polardb_query.reset_wait();
				polardb_query.reader_wait_spec.reset();
				PolarDB_Query_ReaderPlan degraded_plan =
					polardb_query.reader_plan;
				degraded_plan.group_lsn = 0;
				degraded_plan.max_lag_bytes = -1;
				PolarDB_WaitSpec no_wait;
				PolarDB_ReaderResult degraded_result =
					PgHGM->polardb_acquire_reader_connection(
						mybe->hostgroup_id, this,
						degraded_plan, no_wait, false,
						nullptr, -1, confirm_reader_group_capacity);
				if (degraded_result.acquired() &&
						polardb_reader_capacity_wait.active) {
					polardb_leave_reader_capacity_wait(
						PolarDB_ReaderStatus::ACQUIRED);
				} else if (degraded_result.status ==
						PolarDB_ReaderStatus::READER_BUSY ||
					degraded_result.status ==
						PolarDB_ReaderStatus::READER_GROUP_BUSY) {
					polardb_enter_reader_capacity_wait(
						degraded_result.retry_scope_hash,
						degraded_result.status, degraded_result.srv,
						std::move(degraded_result.selected_server_snapshot),
						degraded_result.reservation_profile_generation,
						&degraded_result.reservation_pool_key,
						PolarDB_PoolCapacityTarget::READER,
						&degraded_plan, &no_wait);
				}
				if (degraded_result.acquired()) {
					mc = degraded_result.conn;
				}
				polardb_reader_acquisition_handled = true;
			} else if (polardb_wait_spec.has_wait() &&
					reader_result.status ==
					...
					(polardb_missing_lsn_action_from_int(
						polardb_query.reader_plan.missing_lsn_action) ==
							PolarDB_MissingLsnAction::ERROR)) {
				POLARDB_TRACE(
				...
				thread->note_pool_attempt(true);
				return true;
			} else if (polardb_wait_spec.has_wait() &&
					reader_result.status ==
						PolarDB_ReaderStatus::RFQ_UNAVAILABLE) {
				// action_missing_lsn=primary is the remaining RFQ case.
				int writer_hg = polardb_query.reader_plan.fallback_writer_hg;
				if (!polardb_redirect_to_writer(writer_hg,
						polardb_reader_status_name(reader_result.status))) {
					POLARDB_TRACE(
						"PolarDB consistency: reader acquisition status=%s but "
						"fallback writer is unknown; normal retry path will handle it\n",
						polardb_reader_status_name(reader_result.status));
					polardb_reader_acquisition_handled = true;
				}
			} else {
				...
				const int writer_hg =
					polardb_query.reader_plan.fallback_writer_hg;
				switch (polardb_reader_acquire_action(
						read_fallback, reader_result.status)) {
				case PolarDB_ReaderAcquireAction::USE_PRIMARY:
					if (!polardb_redirect_to_writer(
							writer_hg,
							polardb_reader_status_name(
								reader_result.status))) {
						polardb_reader_acquisition_handled = true;
					}
					break;
				case PolarDB_ReaderAcquireAction::RETURN_ERROR:
					...
					thread->note_pool_attempt(true);
					return true;
				case PolarDB_ReaderAcquireAction::RETRY_READER:
					// Capacity and topology-change outcomes keep the existing
					// reader wait/reservation path.
					polardb_reader_acquisition_handled = true;
					break;
				}
			}
		...
		if (!polardb_writer_acquisition_handled &&
				!polardb_reader_acquisition_handled)
#endif // POLARDB_PROXY
		{
			mc = get_connection_for_current_backend();
		}
...
#if POLARDB_PROXY && POLARDB_DEBUG
	{
		static std::atomic<int> _w_count{0};
		int _wc = _w_count.fetch_add(1, std::memory_order_relaxed) + 1;
		POLARDB_TRACE("[W%03d.000] ======== WIRE ENTRY ======== sess=%p wait_active=%d wait_stage=%d wait_type=%d session_write_lsn=%lu\n",
			_wc, (void*)this, polardb_wait_active() ? 1 : 0,
			(int)polardb_query.wait.wait_stage, (int)polardb_query.wait.spec.type,
...
#if POLARDB_DEBUG
			const bool txn_before =
				transaction_state_manager->is_in_transaction();
			const bool transaction_command =
				transaction_state_manager->handle_transaction(digest_text);
			POLARDB_TRACE(
				"PolarDB TXN_STATE: command=%d before=%d after=%d "
				"status=%d digest='%s'\n",
				transaction_command ? 1 : 0,
				txn_before ? 1 : 0,
				transaction_state_manager->is_in_transaction() ? 1 : 0,
...
#if POLARDB_PROXY
		PGSQL_QUERY_command polardb_query_cmd = PGSQL_QUERY_UNKNOWN;
		const char* polardb_query_text = NULL;
		std::string polardb_query_text_storage;
#endif // POLARDB_PROXY
		switch (status)
		{
		case PROCESSING_QUERY:
			...
			if (query_digest_text) {
				polardb_query_text = query_digest_text;
			} else if (CurrentQuery.QueryPointer && CurrentQuery.QueryLength > 0) {
				polardb_query_text_storage.assign(
					reinterpret_cast<const char*>(CurrentQuery.QueryPointer),
					static_cast<size_t>(CurrentQuery.QueryLength));
				polardb_query_text = polardb_query_text_storage.c_str();
			}
			polardb_query_cmd = CurrentQuery.PgQueryCmd;
#endif // POLARDB_PROXY
			break;
		case PROCESSING_STMT_EXECUTE:
			if (CurrentQuery.extended_query_info.stmt_info) {
				query_digest_text =
					CurrentQuery.extended_query_info.stmt_info->digest_text;
#if POLARDB_PROXY
				if (query_digest_text) {
					polardb_query_text = query_digest_text;
				} else if (CurrentQuery.extended_query_info.stmt_info->query &&
						CurrentQuery.extended_query_info.stmt_info->query_length > 0) {
					polardb_query_text_storage.assign(
				...
				polardb_query_cmd =
					CurrentQuery.extended_query_info.stmt_info->PgQueryCmd;
#endif // POLARDB_PROXY
			}
			break;
		default:
...
__cleanup:
	...
	polardb_clear_request_state_for_query_end(myds, called_on_failure);
#endif // POLARDB_PROXY
