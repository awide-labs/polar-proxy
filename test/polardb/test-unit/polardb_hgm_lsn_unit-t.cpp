...
static bool polardb_plain_send_denied_by_policy(int* denied_errno) {
	int probe_fds[2] = {-1, -1};
	if (socketpair(AF_UNIX, SOCK_STREAM, 0, probe_fds) != 0) {
		return false;
	}
	...
	errno = 0;
#ifdef __APPLE__
	const ssize_t sent = send(probe_fds[0], &byte, 1, 0);
#else
	const ssize_t sent = send(probe_fds[0], &byte, 1, MSG_NOSIGNAL);
#endif
...
static void test_v2_target_reader_keeps_wait_until_lsn_is_reached() {
	...
	const uint64_t stale_gap_bucket_before =
		worker->polardb_status_variables.stvar[
			polardb_st_var_reader_target_gap_le_4kb];
	const uint64_t stale_gap_sum_before =
		worker->polardb_status_variables.stvar[
	...
	ok(worker->polardb_status_variables.stvar[
			polardb_st_var_reader_target_gap_le_4kb] ==
			stale_gap_bucket_before + 1 &&
		worker->polardb_status_variables.stvar[
			polardb_st_var_reader_target_selected_gap_sum_bytes] ==
			stale_gap_sum_before + REQUIRED_READER_LSN - STALE_READER_LSN,
	...
	reader->lsn_updated_at.store(monotonic_time(), std::memory_order_relaxed);
	const uint64_t zero_gap_before =
		worker->polardb_status_variables.stvar[
			polardb_st_var_reader_target_gap_zero];
	...
	ok(worker->polardb_status_variables.stvar[
			polardb_st_var_reader_target_gap_zero] == zero_gap_before + 1,
...
static void test_reader_pool_request_limits() {
	...
	PolarDB_PoolRequest pooled_request =
		polardb_make_read_pool_request(startup_profile, true, false);
	...
	PolarDB_PoolRequest ordinary_request =
		polardb_make_read_pool_request(startup_profile, false, false);
...
static void test_core_match_pool_index_and_transfer() {
	...
	ok(!stale_startup.conn && stale_startup.retry_current_state &&
	...
	ok(!stale_server_list.conn && stale_server_list.retry_current_state &&
...
static void test_multi_reader_selection_samples_two_servers() {
	...
	PolarDB_WaitSpec target_wait = PolarDB_WaitSpec::lsn(
		TARGET_LSN, POLARDB_DEFAULT_WAIT_TIMEOUT_MS,
		PolarDB_WaitMode::BEST_EFFORT);
	PolarDB_ReaderResult target_result = PgHGM->get_MyConn_polardb_reader(
	...
	ok(target_result.acquired() && target_result.srv == target_ready &&
		...
		"PolarDB multi-reader selection: target-ready reader wins across more than two readers");
...
static void test_two_reader_freshest_behind_reader_is_preferred() {
	...
	balanced_sess.thread = balanced_worker.get();
	attach_test_frontend(balanced_sess);
	PolarDB_ReaderResult balanced_result = PgHGM->get_MyConn_polardb_reader(
	...
	reached_sess.thread = reached_worker.get();
	attach_test_frontend(reached_sess);
	PolarDB_ReaderResult reached_result = PgHGM->get_MyConn_polardb_reader(
...
static void test_two_reader_fresher_less_loaded_policy() {
	...
	hostgroup->second.selection_start->store(0, std::memory_order_relaxed);
	PolarDB_ReaderResult less_loaded = PgHGM->get_MyConn_polardb_reader(
		reader_hg, &sess, plan, wait, /*only_pooled=*/false);
	...
	equal_sess.thread = equal_worker.get();
	attach_test_frontend(equal_sess);
	PolarDB_ReaderResult equal_loaded = PgHGM->get_MyConn_polardb_reader(
	...
	more_sess.thread = more_worker.get();
	attach_test_frontend(more_sess);
	PolarDB_ReaderResult more_loaded = PgHGM->get_MyConn_polardb_reader(
	...
	exact_sess.thread = exact_worker.get();
	attach_test_frontend(exact_sess);
	PolarDB_ReaderResult exact_precedence = PgHGM->get_MyConn_polardb_reader(
	...
	equal_lsn_sess.thread = equal_lsn_worker.get();
	attach_test_frontend(equal_lsn_sess);
	PolarDB_ReaderResult equal_lsn = PgHGM->get_MyConn_polardb_reader(
...
static void test_split_warmup_request_dedup() {
	...
	set_lazy_warmup(true);
	const unsigned long long current_state_retry_before =
		PgHGM->status.polardb_reader_pool_current_state_retry.load(
			std::memory_order_relaxed);
	...
	PgHGM->warm_split_pools();
	ok(PgHGM->status.polardb_reader_pool_current_state_retry.load(
			std::memory_order_relaxed) == current_state_retry_before + 1,
...
static void test_startup_config_commit_contract() {
	if (!GloPTH) {
	...
	const char* alternate_consistency =
		after_startup.consistency_mode == POLARDB_CONSISTENCY_LSN ?
			"off" : "lsn";
...
static void test_demand_startup_contract_acceptance() {
	const int writer_hg = 988;
	const int reader_hg = 989;
	stage_polardb_topology(PgHGM, "PolarDB demand startup contract",
		writer_hg, "polardb-contract-writer", 22982,
		reader_hg, "polardb-contract-reader", 22983);
	PgSQL_SrvC* reader = find_pgsql_server(
		PgHGM->MyHGC_lookup(reader_hg),
		"polardb-contract-reader", 22983);
	ok(reader != nullptr,
		"PolarDB demand startup contract: reader fixture is available");
	if (!reader || !GloPTH) {
		return;
	}
	...
	conn.install_polardb_startup_contract(
		...
		startup_client);
	ok(conn.polardb_startup_contract_installed &&
		...
		"PolarDB demand startup contract: installation preserves the selected startup state");
	ok(PgHGM->polardb_connected_reader_accepts_current_startup(
			&conn, reader_hg),
		"PolarDB demand startup contract: unchanged current state accepts the connected reader");

	conn.polardb_startup_config_generation++;
	ok(!PgHGM->polardb_connected_reader_accepts_current_startup(
			&conn, reader_hg),
		"PolarDB demand startup contract: stale startup generation rejects the connected reader");
	conn.polardb_startup_config_generation =
		global_config.startup.generation;
	reader->set_status(MYSQL_SERVER_STATUS_OFFLINE_HARD);
	ok(!PgHGM->polardb_connected_reader_accepts_current_startup(
			&conn, reader_hg),
		"PolarDB demand startup contract: offline selected server rejects dispatch");
	reader->set_status(MYSQL_SERVER_STATUS_ONLINE);
}
...
static void test_hgm_shutdown_releases_server_snapshots() {
	...
	ok(current_snapshot != nullptr,
		"PolarDB shutdown: current server snapshot exists before quiesce");
	current_snapshot.reset();

	PgHGM->shutdown();
	ok(PgHGM->get_polardb_server_list_snapshot() == nullptr,
		"PolarDB shutdown: server snapshot publication is empty after quiesce");
	...
	ok(!PgHGM->is_polardb_hostgroup(1115),
		"PolarDB shutdown: topology access fails closed after quiesce");

	PgHGM->shutdown();
	ok(PgHGM->get_polardb_server_list_snapshot() == nullptr,
		"PolarDB shutdown: repeated quiesce is idempotent");
}
...
int main() {
	...
	test_startup_config_commit_contract();
	test_demand_startup_contract_acceptance();
