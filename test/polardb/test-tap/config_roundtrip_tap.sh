...
admin_sql "INSERT INTO pgsql_replication_hostgroups (writer_hostgroup, reader_hostgroup, check_type, txn_split_enabled, consistency_mode, max_lag_bytes, lsn_wait_timeout_ms, proxy_protocol, comment) VALUES (101, 102, 'polardb', 1, 'lsn', 12345, 0, 'legacy', 'polardb_hg_roundtrip');" >/dev/null
admin_sql "SAVE CONFIG TO FILE $PROXYSQL_CONFIG_FILE;" >/dev/null
admin_sql "DELETE FROM pgsql_replication_hostgroups;" >/dev/null
...
[ "$hg_policy" = "polardb|1|lsn|12345|0|legacy|polardb_hg_roundtrip" ]
...
admin_sql "INSERT INTO pgsql_replication_hostgroups (writer_hostgroup, reader_hostgroup, check_type, txn_split_enabled, consistency_mode, max_lag_bytes, lsn_wait_timeout_ms, proxy_protocol, comment) VALUES (105, 106, 'polardb', 1, 'lsn', -1, 0, 'v15', 'quote_${quote_payload_sql}_roundtrip');" >/dev/null
admin_sql "SAVE CONFIG TO FILE $PROXYSQL_CONFIG_FILE;" >/dev/null
admin_sql "DELETE FROM pgsql_replication_hostgroups;" >/dev/null
admin_sql "LOAD PGSQL SERVERS FROM CONFIG;" >/dev/null
...
ok $? "PolarDB startup and reader-selection globals load to runtime"
...
set_global_var pgsql-polardb_proxy_identity_mode bogus
admin_sql "LOAD PGSQL VARIABLES TO RUNTIME;" >/dev/null
[ "$(runtime_var pgsql-polardb_proxy_protocol)" = "legacy" ] &&
	[ "$(runtime_var pgsql-polardb_route_rfq_policy)" = "best_effort" ] &&
	[ "$(runtime_var pgsql-polardb_session_lsn_baseline)" = "primary" ] &&
